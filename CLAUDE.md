...
**Phase dumps** (show compiler IR at each stage): `ORI_DUMP_AFTER_PARSE=1` (AST) | `ORI_DUMP_AFTER_TYPECK=1` (typed IR) | `ORI_DUMP_AFTER_ARC=1` (ARC IR with RC strategies) | `ORI_DUMP_AFTER_LLVM=1` (annotated LLVM IR, superset of `ORI_DEBUG_LLVM`) | `ORI_EMIT_ARC_DOT=1` (ARC IR as GraphViz DOT: `ori build file.ori 2> arc.dot && dot -Tsvg arc.dot -o arc.svg`) — all output to stderr, zero overhead in release builds.
