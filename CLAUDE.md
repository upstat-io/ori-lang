...
- **INVERTED-TDD is BANNED** — never "make tests pass" by neutering the deliverable on the failing path. Adding a gate, flag, early-return, exemption, or `#cfg` skip that short-circuits a validator/check/assertion/invariant for "problematic" inputs is INVERTED-TDD — the tests pass because what they were designed to catch has been disabled. Canonical example: a PC-2 validator with `if sig.scheme_var_ids.is_empty() { return; }` added to silence failing generic-body tests — the validator now fails to enforce PC-2 on exactly the generic bodies it was designed to cover. When a subsection's stated deliverable is a validator/check/enforcement, and tests fail on inputs the deliverable should be active on, the correct response is to fix the code the deliverable catches, NOT to disable the deliverable. Banned signals in commits / plan updates / code comments: "make tests pass", "pragmatic workaround", "gate the failing path", "accept current state and proceed", "file bugs and proceed" (without per-bug `/fix-bug` vs `/add-bug` classification), "mark as Known Failing Tests" (without a concrete `- [ ]` anchor per §ALL Deferrals). Reviewers (`/tpr-review`, `/review-work`, `/impl-hygiene-review`, `/review-plan`) MUST flag INVERTED-TDD as Critical per `impl-hygiene.md §Finding Categories` and block section close-out. The remediation is always the architecturally-correct fix per §The One Rule, regardless of scope/effort. When a blocker bug surfaces during subsection execution, invoke `/fix-bug` NOW (not `/add-bug` followed by "proceed") and complete the fix with full plan-section rigor before resuming.
- **Invariant-anchoring before test-chasing** — before writing or modifying code that touches a validator/check/invariant/enforcement, re-state in plain text: (a) what system invariant this enforces (spec clause, `typeck.md §PC-*`, `aims-rules.md` lattice dimension, AIMS invariant, phase contract), (b) which downstream subsystem consumes that invariant, (c) if tests fail, whether the correct fix is in the code-under-test or in the invariant itself. The answer to (c) is almost always the code under test. "Narrow-scope goal drift" — losing sight of the system invariant a subsection serves while optimizing for local green tests — is the root cause of INVERTED-TDD. Forcing explicit invariant-anchoring makes goal drift visible at decision time.
...
- **Hook failure = mandatory re-grounding, not a workaround hunt** — when `/commit-push` fails because a pre-commit hook rejected the commit, **STOP**. Do not touch any file yet. A hook failure is a context-collapse event: the goal displaces from "have correct code" to "make the commit succeed." Before any other action, re-ground: (1) re-read the active task document (fix section file or plan section) — what is the stated deliverable and what invariant is it enforcing? (2) re-read §The One Rule above; (3) re-read impl-hygiene.md. Only after re-grounding, read the hook output fresh and fix what it caught — correctly. Banned responses to hook failure (all = bypass): `#[allow(clippy::...)]`, weakened tests, gates/flags/early-returns that silence the failing path, `#[ignore]` on tests, `--no-verify`, narrowing coverage to avoid the failing case. If the correct fix is large, that is not a reason for a shortcut — the size IS the work. If genuinely blocked, use `AskUserQuestion` immediately.
...
**Plan corpus**: `python -m scripts.plan_corpus check <path>` (validate plan frontmatter + body — including §06.2 `## Intelligence Reconnaissance` block for PLAN_SECTION files) | `check --strict-recon` (escalate `status: not-started` missing/stub recon findings from Outcome.WARNING to Outcome.ERROR; `in-progress` unaffected; exempt classes unchanged) | `discover` (full corpus report with per-plan recon coverage table) | `docgen` (generate schema reference) | `docgen --check` (drift gate). Exit policy: returns 1 iff any finding has `Outcome.ERROR`; `Outcome.WARNING` findings print but do not gate. Severity (`LOW`/`MEDIUM`/`HIGH`/`CRITICAL`) and Outcome (`WARNING`/`ERROR`) are independent axes.
