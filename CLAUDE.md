...
- **ZERO DEFERRAL on bugs** — when you discover a bug, issue, leak, performance problem, or anything broken: (1) IMMEDIATELY add it to your todo list, (2) fix it NOW in this session using `/fix-bug` (creates a fix section file with plan-section rigor: root cause analysis, TDD matrix, completion checklist including TPR + hygiene review). No "tracked for later", no "known issue", no "pre-existing", no "separate fix needed". The discovery IS the assignment. There are zero valid reasons to defer — not scope, not complexity, not "it was already broken". If you found it, you fix it. **Pre-existing bugs MUST be fixed immediately** — "pre-existing" is diagnosis only, never justification for skipping.
...
**NEVER TIMEOUT REVIEWS/AGENTS**: Do NOT apply timeouts to `/tpr-review`, `/tp-help`, `codex exec`, `/review-work`, `/independent-review`, Agent tool tasks, or any non-test long-running process. No `timeout` prefix, no Bash `timeout:` parameter, no Agent timeout. These are review/analysis tasks that legitimately take 5-15 minutes.
