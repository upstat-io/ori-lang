...
12. **Run `/improve-tooling` retrospectively after both reviews are clean — MANDATORY at every section close** — Once `/tpr-review` and `/impl-hygiene-review` both pass clean, invoke `/improve-tooling` in **retrospective mode** (see the skill's "Retrospective Mode" section). This is a deliberate look-back at the section's debugging journey, NOT a reactive friction check. Reconstruct what you actually did: which `diagnostics/` scripts you ran, which command sequences you repeated, where you added `dbg!`/`tracing` calls to figure something out, where you stared at output trying to interpret it, where test failures gave unhelpful messages, where you mentally cross-referenced files because no tool combined them. Then ask: "what tool/log/diagnostic improvement would have made this section materially easier — and would help the next section that touches this area?" Implement every accepted improvement NOW (zero deferral — the improvement IS section close-out work). Commit improvements via `/commit-push` as **separate commits** from the section's implementation work, with messages like `tools(diagnostics): add --per-block flag to codegen-audit.sh — surfaced by section-NN retrospective`. Verify each improvement actually solves the friction by re-running the original workflow against the improved tool. **Why this matters:** the diagnostic suite cannot be designed up front for every use case — it grows organically by accumulating one targeted improvement per section, guided by what the work actually demanded. Skipping the retrospective lets small frictions normalize and disappear from memory; capturing them at section boundaries is how the tooling stays sharp. The retrospective is mandatory even when nothing *felt* painful — that is exactly when blind spots accumulate. If genuinely no improvement is warranted (rare), document the negative finding briefly: "Retrospective: no tooling gaps — section relied entirely on existing scripts X, Y which were sufficient." Do not silently skip.
...
- [ ] Run `/improve-tooling` retrospectively — MANDATORY at every section close, after both reviews are clean. Look back at the section's debugging journey (which diagnostic scripts you ran, which command sequences you repeated, where you added ad-hoc `dbg!`/`tracing` calls, where output was hard to interpret) and identify any tool/log/diagnostic improvement that would have made this section materially easier or that would help the next section touching this area. Implement every accepted improvement NOW (zero deferral) and commit each as a **separate** commit via `/commit-push` (e.g., `tools(diagnostics): add --per-block to codegen-audit.sh — surfaced by section-NN retrospective`). Verify each improvement actually solves the friction. The retrospective is mandatory even when nothing felt painful — that is exactly when blind spots accumulate. If genuinely no gaps, document it briefly: "Retrospective: no tooling gaps". Do not silently skip.
