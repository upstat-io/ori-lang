...
3. **If no owning plan exists — file as a bug in `plans/bug-tracker/`** under the appropriate subsystem section using the reviewer-tagged IDs. Use the BUG entry format, not the TPR entry format:
   ```md
   - [ ] `[BUG-{section}-{ordinal}-codex][{severity}]` **{Short title}** — found by review-work.
     Repro: {from Codex output}
     Subsystem: {crate/file path}
     Found: {YYYY-MM-DD} | Source: review-work
   - [ ] `[BUG-{section}-{ordinal}-gemini][{severity}]` **{Short title}** — found by review-work.
     Repro: {from Gemini output}
     Subsystem: {crate/file path}
     Found: {YYYY-MM-DD} | Source: review-work
   ```
...
- Mark the filed findings as `[x]` resolved in the plan (or bug-tracker) with a note referencing the code fix:
  ```md
