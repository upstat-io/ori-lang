...
Bash (run_in_background: true):
  rm -f /tmp/review-work.jsonl /tmp/review-work.done
  codex exec "run the /review-work skill" --full-auto --json 2>/dev/null > /tmp/review-work.jsonl
  ec=$?
  touch /tmp/review-work.done
  echo "exit=$ec"
```
...
from the JSONL output — the last `agent_message` item contains the
findings summary.
...
import json
msgs = []
with open('/tmp/review-work.jsonl') as f:
    for line in f:
        line = line.strip()
        if not line:
            continue
