...
**Tooling grows organically.** You cannot predict every use case ahead of time. The way the diagnostic suite gets sharp is by ratcheting it up by one improvement after every section, every bug fix, every debugging session — guided by what was *actually* painful, not what was imagined to be painful. This skill has two trigger modes: **reactive** (mid-task friction, the original auto-trigger) and **reflective** (post-section retrospective — see Retrospective Mode below).
...
## Retrospective Mode (Section Boundaries)

**Triggered by `/continue-roadmap`, `/create-plan`, and any plan workflow at the end of a section** — after `/tpr-review` and `/impl-hygiene-review` are clean, but before the section is marked complete.

This mode is **reflective, not reactive**. It runs even when nothing felt blocked during the section. The premise: small frictions normalize and disappear from memory, so you must capture them while the section's debugging journey is fresh. Tooling cannot be designed up front for every use case — it grows by accumulating one targeted improvement per section, guided by what the work actually demanded.

### Retrospective Workflow

When invoked retrospectively at the end of a section:

1. **Reconstruct the section's debugging journey.** Look at the section's commits, the test files added, the diagnostic scripts run, the IR/RC dumps inspected, the manual steps taken. Ask:
   - Which `diagnostics/` scripts did I run? How many times? Did I have to pipe/grep/manually parse output?
   - Which command sequences did I repeat? (e.g., "build, run with `ORI_DUMP_AFTER_ARC=1`, grep for the function, eyeball the IR")
   - Where did I add `dbg!` / `eprintln!` / `tracing::debug!` to figure something out?
   ...
   - Which symptoms required cross-referencing multiple files mentally because no tool combined them?
   - Did I write any one-off shell incantations a script should own permanently?

2. **Forward-look as well as back-look.** Ask: "If someone hits a regression in this code path next month, what tool/log/diagnostic would shorten their debugging session by 10 minutes?" Add it now, while the context is fresh — not when the regression actually happens.

3. **List concrete improvement candidates.** For each item, articulate:
   ...
   - **Payoff**: how it would have shortened *this* section's work, or how it sharpens future debugging

4. **Filter brutally.** Not every small annoyance becomes a tool change. Apply this filter:
   - **DO improve** if the friction would recur: same workflow on similar bugs, same script run by other sections, same output format misread by future implementers
   ...
   - **DO NOT improve** if the friction was a one-off due to unique section content with no recurring pattern
...
8. **Update documentation** — `CLAUDE.md` if the tool is listed there, the script's `--help`, `diagnostics/README.md` for diagnostic additions.

### Anti-Patterns Specific to Retrospective Mode

- **"Nothing was painful, skipping retrospective."** — The retrospective is mandatory at section close, not optional. The fact that nothing *felt* painful is exactly why the look-back is needed: small frictions become invisible. Force yourself to enumerate the actual commands run; gaps will surface.
...
- **"This is a one-off, no future section will need it."** — Be honest. If you genuinely can't articulate a recurring use case, skip it. But "one-off" is often a rationalization — most debugging patterns recur.
- **Combining tooling improvements into the section's main commit.** — Separate commits keep provenance clean and let `/improve-tooling` retrospectives be reviewed independently of feature work.
...
Retrospective mode covers all three. It's the difference between a tooling suite that grows by accident and one that grows on purpose.
