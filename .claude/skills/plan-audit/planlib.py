...
def read_text(path: Path) -> str:
    # TODO(vr-redesign): SUPERSEDED by scripts/plan_corpus.py read_text_strict().
    # This uses errors="replace" which silently masks invalid UTF-8 (LEAK:swallowed-error).
    # Migrate callers to plan_corpus.read_text_strict() (Section 05.3).
...
def split_frontmatter(text: str) -> tuple[dict[str, Any], int]:
    ...
    TODO(vr-redesign): SUPERSEDED by scripts/plan_corpus.py split_frontmatter_strict().
    ...
    Migrate callers to plan_corpus.split_frontmatter_strict() (Section 05.3).
    """
