...
- **Gemini** (`gemini --approval-mode yolo --output-format stream-json ...` — no `--model` flag; the CLI auto-selects a model from the user's configured tier so Pro quota is only consumed when Google's routing decides a prompt warrants it)
