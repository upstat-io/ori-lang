...
1. **Structural ARC IR verification** (`pipeline::run_verify` → `verify::check_function`) — detects use-before-def (`VerifyError::UseBeforeDef`), dangling block references (`DanglingBlockRef`), RcInc/RcDec on scalar-repr variables (`RcOnScalar`), RcDec on borrowed parameters (`DecOnBorrowed`), arg_ownership length mismatches (`ArgOwnershipLenMismatch`), and wrapped FIP structural errors (`FipStructural`). It does NOT check "RC balance" or "drop placement" as holistic properties — each variant targets a specific well-formedness failure. Called at exactly two checkpoints: after AIMS emission and after the full AIMS pipeline (`pipeline/aims_pipeline/postprocess.rs:21,63`); NOT after every pipeline step.
...
4. **FIP certification** (`aims::verify::fip::verify_fip_contract`, `aims/verify/fip.rs`) — proves `FipContract::Certified` functions have zero unmatched allocations/deallocations in the realized IR.
