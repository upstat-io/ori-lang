...
2. **RC balance**: `ORI_CHECK_LEAKS=1` reports zero leaks AND `ORI_TRACE_RC=1` shows balanced inc/dec
3. **IR-level verification** (when touching `ori_arc` or `ori_llvm`): use `ORI_DUMP_AFTER_ARC=1` or `ORI_DUMP_AFTER_LLVM=1` to verify exact RC operation placement. Document expected operations in test comments.
...
**Regression test naming**: use descriptive names, not bare issue numbers. `test_cow_slice_double_free_nested_list` not `test_issue_42`. The name should describe the regression scenario so future readers understand what's being guarded without looking up the issue. (From Rust's tidy enforcement of descriptive naming.)
