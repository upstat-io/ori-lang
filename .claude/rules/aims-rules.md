...
**IC-7** — Convergence: finite domain guarantees termination. The iteration limit SHALL be derived from the domain heights: parameter contract 6 dimensions (access=1 + consumption=3 + cardinality=2 + locality=4 + uniqueness=2 + may_share=1 = 13 per param), return contract 4 dimensions (uniqueness=2 + freshness=1 + locality=4 + shape=1 = 8 total), EffectSummary 6 boolean fields, ContextBehavior 4 boolean fields (PL-7/PL-11), FipContract (IC-6): `Never < Certified < Bounded(n) < Conditional` — height 3 (the `Bounded(n)` form has a FINITE upper bound per function: `n ≤ alloc_count(function)`, the number of allocation instructions; `Bounded(n) ⊔ Bounded(m) = Bounded(max(n,m))` which is monotonically non-decreasing and bounded). Formula: `param_count × 13 + 8 + 6 + 4 + 3`. If exceeded, widen all contracts to most conservative and emit a diagnostic. This document (`aims-rules.md`) is authoritative for the convergence bound; `arc.md` references it.
...
- **`Project` (borrow — NOT a transparent alias)**: the destination's demand does NOT transfer full dimensions to the source. Only LOCALITY is propagated (via TF-14: `src.locality := max(src.locality, dst.locality)`). The source aggregate is consumed Once by the Project (TF-11: `(value, Once)`), regardless of how many times the projected field is used downstream. Propagating `Many` cardinality from a heavily-used field back to the parent would be incorrect — the parent is accessed Once by the Project instruction.
...
**RL-15a** — ArgEscaping allocations (`Locality = ArgEscaping ∧ Uniqueness = Unique`) SHALL be stack-allocated in the CALLER. The caller's stack frame strictly outlives the callee's execution, so the allocation survives the callee's use. Uniqueness is preserved (CN-6 does not fire for ArgEscaping — only HeapEscaping and above). No RC header is needed because `Unique` guarantees no aliasing. The `Uniqueness = Unique` requirement is the same as RL-14 — a `MaybeShared` `ArgEscaping` value needs a header for `IsShared` checks. This is the key optimization that bridges "not local" and "not heap" — a value that escapes into a callee but not to the heap gets caller-stack lifetime without heap overhead.
