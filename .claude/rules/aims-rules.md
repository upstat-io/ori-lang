...
**Propagation scope — why TF-14 and project_alias_sources propagate different things**: TF-14 (direct borrow sources) propagates locality AND cardinality: `src.locality := max(src.locality, dst.locality)` and `src.cardinality := max(src.cardinality, dst.cardinality)`. Locality prevents premature stack promotion; cardinality prevents premature RcDec (keeps the source alive as long as the borrow is alive). TF-14 does NOT propagate consumption or other dimensions — the source's consumption tracks its own usage pattern, independent of how the borrow is used. In contrast, `project_alias_sources` propagates borrowed demand with Locality and Cardinality (but NOT Consumption — matching IA-5 step 1 and TF-14: the source's consumption tracks its own usage pattern) because transitive aliases cross block boundaries where the standard per-instruction backward demand system cannot propagate — a block parameter receiving a projected value via Jump does not know its definition chain, so the project_alias_sources consumer must inject the full "this source is alive through a borrow" signal at block entry.
...
| `Select { true_val, false_val }` | `join(true_path, false_path)` (scalar operands excluded per TF-8; mixed scalar/non-scalar → inherit non-scalar with MaybeShared) | `AND(true_path, false_path)` | `join(true_path, false_path)` | `join(true_path, false_path)` |
| Parameter (direct or via chain of `Let Var` aliases) | `MaybeShared` | `true` | `HeapEscaping` | `NonReusable` |
...
- Parameters traced directly or through a SINGLE `Let Var(param)` alias get `preserves_freshness = true` — meaning IF the CALLER passes a fresh value, the callee preserves that freshness property in the return (compositional). `preserves_freshness` is NOT "this return IS fresh" — it is "freshness passes through this function." RL-29 requires BOTH `preserves_freshness = true` AND `uniqueness = Unique` for `noalias`: the uniqueness gate catches non-unique parameters, and the freshness gate ensures the compositional chain traces back to a fresh allocation.
...
| `Apply { args }` | `(arg, Once)` per arg; refined by callee contract (IC-3): `Absent` parameter → zero demand; `arg.locality := max(arg.locality, param.locality)` (callee parameter locality widening); `arg.access := max(arg.access, param.access)` (Owned callee param forces Owned arg) |
...
**VF-1** — Layer 1 (Structural): ARC IR well-formedness. Checks: use-before-def, dangling block refs, RC on scalar, dec on borrowed, arg ownership length mismatch. Runs at three checkpoints: (1) after AIMS emission (Step 6), (2) after full pipeline (Step 11), (3) after post-pipeline optimization passes (§8 RL-22 through RL-26).
