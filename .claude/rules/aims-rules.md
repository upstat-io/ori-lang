...
**Consumer**: `propagate_project_source_demand()` — invoked on EVERY iteration of the backward worklist (inside `compute_block_entry_state()`), not as a post-convergence pass. On each worklist iteration, for each alias variable in the current block-entry demand map that is live (Cardinality ≠ Absent), propagates an `Owned` demand to ALL source aggregates in the map: `Access = Owned` (needed for field cleanup RcDec per RL-14), `Locality = max(source_current, alias_locality)`, `Cardinality = seq_add(source_current, alias_cardinality)`. Consumption is NOT propagated. Because this runs inside the worklist loop, the propagated demand feeds back into predecessor exit states on subsequent iterations, ensuring cross-block convergence.
...
| `Apply { args }` | `(arg, cardinality=Once, consumption=Linear)` per arg; refined by callee contract (IC-3): `Absent` parameter → zero demand; `arg.locality := max(arg.locality, max(param.locality, ArgEscaping))` for Owned params (arguments to Owned params always cross the function boundary — DP-8 excludes ArgEscaping from `is_local()` to prevent unsound headerless stack allocation); `arg.access := max(arg.access, param.access)` (Owned callee param forces Owned arg) |
...
**TF-14** — Project backward demand propagation: when a projected variable `dst` accumulates demand, that demand SHALL be propagated to the borrow source `src` via `borrow_sources`: `src.access := Owned` (unconditional — needed for field cleanup RcDec per RL-14), `src.locality := max(src.locality, dst.locality)`, `src.cardinality := seq_add(src.cardinality, dst.cardinality)`. This matches the project_alias_sources consumer (§1.9) semantics — both TF-14 (intra-block) and project_alias_sources (cross-block) use the same Owned/locality/cardinality propagation. Without this propagation, `src` can be freed or stack-allocated while a reference to its projected field escapes or outlives it.
...
- **`Let { Var(v) }` (transparent alias)**: the destination's accumulated demand transfers to the source. Cardinality and Consumption use `seq_add` (Once + Once = Many, Linear + Linear = Affine — additive resource counting). Locality uses `max` (join). This ensures a variable used both directly AND through an alias is correctly counted as Many. E.g., if `%4 = Let Var(%3)` and `%4` has `Many` cardinality from 3 uses, then `%3` inherits `Many`. This is the backward interpretation of TF-2: `dst := state(src)` means `src` should absorb `dst`'s demand.
- **`Project` (borrow — NOT a transparent alias)**: `src.access := Owned` (unconditional — needed for field cleanup RcDec), `src.cardinality := seq_add(src.cardinality, dst.cardinality)` (keeps source alive as long as borrow is alive), `src.locality := max(src.locality, dst.locality)` (via TF-14). Consumption is NOT transferred. This matches TF-14 and project_alias_sources consumer semantics — all three use the same Owned/locality/cardinality propagation for consistency. The standard TF-11 demand `(value, cardinality=Once, consumption=Linear)` additionally records the Project's direct operand use.
...
**RL-14a** — Non-escaping fixed-size allocations with `Uniqueness ≠ Unique` (`Locality ≤ FunctionLocal`) SHALL be stack-allocated via `alloca` WITH RC header (needed for `IsShared` checks). **Heap children**: field-level `RcDec` at scope exit per RL-14.
...
- **Callee parameter = `Borrowed` AND `Uniqueness = Unique`**: headerless stack allocation (no RC operations emitted by the callee). **Heap children**: field-level `RcDec` at caller scope exit per RL-14 (VF-1 exemption applies). This is the fast path — caller stack lifetime with zero RC overhead on the container.
...
| `MaybeShared ∧ BlockLocal` | Fresh values retain BlockLocal if single-block (IA-4); MaybeShared from join with a MaybeShared path or from callee return contract (TF-6 refine) |
