...
- `Set { dst, obj, field, val }`: field mutation. `dst` = the (possibly COW-copied) result. Forward transfer: `dst := state(obj)` (inherits the base's state — COW slow path produces a copy with the same ownership). Backward demand: `(obj, Once, Linear)` + `(val, Once, Linear)`.
- `SetTag { dst, obj }`: tag mutation. `dst` = result. Forward: `dst := state(obj)`. Backward: `(obj, Once, Linear)`.
...
- **`Project` (borrow — NOT a transparent alias)**: `src.cardinality := seq_add(src.cardinality, dst.cardinality)`, `src.locality := max(src.locality, dst.locality)` (via TF-14). No access promotion — CN-8 clamps Borrowed states to FunctionLocal, so escaping borrow destinations are unreachable. Owned promotion for heap-escaping values happens at the ESCAPING instruction (Construct/Set/Return/TF-13). The standard TF-11 demand `(value, cardinality=Once, consumption=Affine)` records the Project's direct operand use.
