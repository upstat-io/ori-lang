...
| `Apply { args }` | `(arg, cardinality=Once, consumption=Linear)` per arg; refined by callee contract (IC-3): `Absent` → zero demand; Owned params: `arg.locality := max(arg.locality, max(param.locality, ArgEscaping))`, `arg.access := Owned`; Borrowed params with `may_share = true`: `arg.uniqueness := MaybeShared` (callee may RcInc, needs header) |
