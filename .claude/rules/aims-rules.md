...
- Deallocation: `may_deallocate = true` when the function's callee contracts include any callee with `may_deallocate = true`, OR the function takes Owned parameters (which receive an RcDec at exit that may trigger free), OR the function's analysis shows any variable transitioning from alive to dead with `access = Owned` (indicating a potential dec that triggers free). Note: this is computed from ANALYSIS STATE and CALLEE CONTRACTS, not from literal `RcDec` instructions (which don't exist at Step 1 before realization). Conservative: any ambiguity resolves to `true`.
...
- `alloc_only_on_slow_path`: AND — computed POST-REALIZATION (after Step 10), not during Step 1. This field is derived from the realized ARC IR where COW branches (`IsShared` + `Branch`) exist. It is `true` only when every allocation in the realized IR is dominated by a COW slow path. At Step 1 time, this field is initialized to `true` (optimistic) and refined after realization. Per-instruction classification (post-realization): unconditional `Construct`/`Reuse`/`CollectionReuse` → `false`; allocation inside a COW slow-path arm → `true`; callee with `alloc_only_on_slow_path = false` → `false`.
...
**RL-12** — Cross-block reuse: a dying value's allocation SHALL be reused across blocks when the death block dominates the allocation block AND the allocation block post-dominates the death block, and the dying value is statically unique. Cross-block reuse uses the SAME `Reset`/`Reuse` instruction pair as RL-11, but the pairing is NOT same-block — the `Reset` is placed at the death point and the `Reuse` at the allocation point across blocks. Token liveness between the Reset and Reuse is guaranteed by the dominance/post-dominance precondition (every path from Reset to Reuse executes both), so TF-10a's same-block structural guarantee does not apply. The RL-11 "no intervening throwing/allocating instructions" constraint is replaced by the dominator-based guarantee that all paths between Reset and Reuse are token-safe. TF-11's token demand suppression (SCALAR, L-9 exclusion) still applies — the analysis does not track cross-block token liveness; the dominance analysis provides it.
...
- **Closure capture (PartialApply)**: when a variable achieves `ArgEscaping` via TF-13 (captured by an ArgEscaping closure) rather than via a direct function call, there is no callee ParamContract to consult. In this case, the allocation strategy is: caller-stack allocation WITH RC header (conservative). The closure environment will increment the captured variable's RC (TF-13 promotes access to Owned), so the header is required.
