...
**RL-26** — RC motion SHALL NOT move operations across RC-observable barriers: (a) calls where the callee has any `Owned` + non-`Dead` parameter, OR any `Borrowed` parameter with `may_share = true` (same as RL-27); (b) `IsShared` instructions (reads the RC header — moving RcInc past IsShared changes the uniqueness observation); (c) `Set`/`SetTag` (implicit field drops may deallocate). Pure calls (all Borrowed + no may_share) are transparent.
