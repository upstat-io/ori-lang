...
- **Closure capture (PartialApply)**: when a variable achieves `ArgEscaping` via TF-13 (captured by an ArgEscaping closure) rather than via a direct function call, there is no callee ParamContract to consult. TF-13 does NOT promote access to Owned for ArgEscaping closures (only >= HeapEscaping). The captured variable stays Borrowed, so no RC increment occurs. Allocation strategy: caller-stack allocation, headerless (captures are Borrowed, closure on caller's stack outlives the callee, no RC operations needed).
