...
**RL-15a** — ArgEscaping allocations (`Locality = ArgEscaping ∧ Uniqueness = Unique`) SHALL be stack-allocated in the CALLER, **provided the callee's parameter contract for the corresponding argument is `Borrowed`** (no RC operations emitted by the callee). If the callee's parameter is `Owned` (the callee may decrement or transfer ownership), stack promotion is UNSAFE — the callee would emit RC operations on a headerless stack allocation. The caller's stack frame strictly outlives the callee's execution, so the allocation survives the callee's use. Uniqueness is preserved (CN-6 does not fire for ArgEscaping — only HeapEscaping and above). No RC header is needed because `Unique` + `Borrowed` callee parameter guarantees no RC operations. This is the key optimization that bridges "not local" and "not heap" — a value that escapes into a callee but not to the heap gets caller-stack lifetime without heap overhead.
