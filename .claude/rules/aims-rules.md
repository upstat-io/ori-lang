...
- **§1.9 `project_alias_sources`**: spec defines 7 rules (direct Project roots, Let aliases, Jump, CFG merge, Select, nested Projects, Set/SetTag). Shipped: implements Rules 1 (direct Project, but without Let-alias-to-root tracing), 2 (Let), 3 (Jump), 4 (CFG merge — unions Jump-source sets across predecessors), 6 (nested Projects partially). Not shipped: Rule 5 (Select conditional aliases), Rule 7 (Set/SetTag mutation tracking). DP-5's full borrow+alias safety and RL-31 disjointness proof depend on unshipped rules.
...
Order: `Dead < Linear < Affine < Unrestricted`. Join: `max`. Height: 3. Note: `join(Dead, Linear) = Linear`. **DP-7 path-sensitivity note**: within a single callee, `(Dead, Absent) ⊔ (Linear, Once) = (Linear, Once)` at a CFG merge point. This converged state passes DP-7's gates, but the parameter is only consumed on ONE path and dead on the other. The soundness argument has two layers: (1) **Intra-callee**: on the Dead path, the callee does NOT emit a dec for this parameter (Dead = no RC obligation). The caller's skipped inc is matched by the callee's skipped dec — the pair cancellation holds because NEITHER operation exists on the Dead path. On the Linear path, ownership transfers via move semantics — the single use consumes the parameter without inc/dec. (2) **Inter-callee (SCC fixpoint)**: IC-2/IC-3 joins parameter states across ALL call sites. If any caller passes an argument that the callee uses conditionally, the converged parameter contract reflects the maximum demand across all callers AND all internal paths. DP-7 is evaluated on the CALLEE's parameter state, which is the join of all internal paths — it represents the worst-case demand. The safety requires that the callee NEVER emits a dec on the Dead path (which is guaranteed by DP-2: `Absent → is_rc_dec_unnecessary = true`).
...
**L-9** — `SCALAR` is a sentinel, NOT a lattice element. The lattice join `a ⊔ SCALAR` is undefined — SCALAR does not participate in lattice operations. Analysis SHALL exclude scalars from the state map. **Transfer-function scalar filtering**: TF-8 (`Select`) handles mixed SCALAR/non-SCALAR operands by EXCLUDING the SCALAR operand before any join — this is a pre-filter, not a lattice join with SCALAR. The exclusion produces the non-SCALAR operand's state (with uniqueness adjusted per TF-8). This is consistent with L-9: the join never sees SCALAR; it is filtered out first.
...
NON-BORROW aliases (Rules 2, 3, 5, 7 — Let Var, Jump, Select, Set/SetTag) do NOT trigger the `propagate_project_source_demand()` mechanism — they exist in `project_alias_sources` for DP-5 safety checks only (checking whether mutations overlap with live aliases). Their backward DEMAND is handled by IA-5 step (1) transparent-alias transfer (Let Var, Select) or by TF-11 Jump argument demand `(arg, Once)` propagated to the successor block parameter. Jump/block-param aliases propagate cross-block via `project_alias_sources` Rule 3 (the alias map extends the chain), while their demand propagation is per-block via TF-11 + IA-4 cross-block widening.
...
**TF-10a** — Reset (`Reset`): `dst := SCALAR` (the reuse token). *Rationale*: The reuse token is an opaque handle to the dead value's memory — it is NOT a refcounted allocation itself. It is consumed exactly once by the corresponding `Reuse` instruction. For **same-block reuse** (RL-11): the pairing is validated structurally at emission time — every Reset has exactly one Reuse in the same block, no intervening throwing instructions. For **cross-block reuse** (RL-12): the token crosses blocks but its availability is guaranteed by dominance/post-dominance analysis with a no-throw constraint; the safety model differs from same-block pairing. **Token liveness model**: tokens are SCALAR for RC purposes (no RC operations), but the backward demand system emits `(token, Once)` at the Reuse instruction (see TF-11 Reuse entry) to prevent dead-code elimination. This demand has no RC effect (L-9 excludes scalars from RC analysis) but ensures the token appears in the demand map for structural pairing validation. The safety guarantee comes from emission-time structural validation (same-block) or dominance analysis (cross-block), not from the demand system — the demand is a secondary safety net.
...
**TF-14** — Project backward demand propagation: `src.locality := max(src.locality, dst.locality)`, `src.cardinality := seq_add(src.cardinality, dst.cardinality)`, `src.consumption := seq_add(src.consumption, Affine)` (keeps source alive AND counts the borrow as an additive use — `seq_add` is consistent with TF-11's QTT semiring resource counting; `max` would undercount when the source is already Linear from another use, giving Affine instead of the correct Unrestricted). No access promotion. Without this propagation, `src` can be freed or stack-allocated while a reference to its projected field escapes or outlives it.
...
1. **Direct borrows**: for all `b` in `borrow_sources` where `borrow_sources[b].source = var`: `b` is NOT live at `point`, OR `borrow_sources[b].path` is disjoint from the mutation's field path. Disjointness for paths: two paths are disjoint if neither is a prefix of the other (per §1.9 BorrowSource definition, paths are `[field_chain]` from root to borrow). First-level fields are a special case where the path has length 1.
...
- `MaybeShared AND parameter with COW-aware caller uniqueness guarantee (IC-3 ParamContract.uniqueness = Unique) ⟹ StaticUnique` (caller proved RC == 1 at entry — this is a PAST guarantee from the interprocedural contract, not a future-use inference). **Safety argument**: (a) **Callee-local aliases** (Let Var) are transparent and don't increment RC — the mutation is visible through the alias, which is correct for value semantics with in-place unique mutation. Project borrows are caught by DP-5 (overlap check). (b) **Caller-retained aliases**: if a caller retains a live alias of the argument (e.g., `let y = x; foo(x)`), the caller's backward analysis gives `x` cardinality Many (used by both Let and Apply). With Many cardinality, the caller emits RcInc before the call, making the argument MaybeShared at the call site. IC-3's fixpoint then converges to `ParamContract.uniqueness = MaybeShared`, NOT Unique — so this DP-9 subcase does not fire. The IC-3 fixpoint prevents unsound uniqueness claims when callers retain aliases.
...
- **Callee parameter = `Borrowed` AND `Uniqueness ≠ Unique`** (MaybeShared or Shared): caller-stack allocation WITH RC header initialized to `MAX_REFCOUNT` (immortal). The callee may emit `IsShared` checks (DP-4), which read the header. Headerless allocation would crash. `MAX_REFCOUNT` prevents the callee's potential `RcDec` from freeing the stack pointer. **Heap children**: field-level `RcDec` at caller scope exit per RL-14.
- **Callee parameter = `Owned`**: caller-stack allocation WITH RC header, initialized to `MAX_REFCOUNT` (immortal). The callee may emit `RcDec` or `IsShared`, so the header is required. The immortal RC prevents the callee's `RcDec` from triggering free() on a stack pointer — dec on an immortal value is a no-op. The caller-stack lifetime still outlives the callee's execution. **Heap children**: field-level `RcDec` at caller scope exit per RL-14 — the callee's `RcDec` on the Owned parameter is a no-op (immortal), so the caller is responsible for field cleanup.
...
"Active direct borrow" checked via `borrow_sources`: entries where `borrow_sources[b].source = var` and `b` is live at `point` and `borrow_sources[b].path` overlaps the mutation's field path (neither is a prefix of the other). For `SetTag`: the mutation affects ALL fields (tag change invalidates all payload fields) — `SetTag` always blocks when any borrow is live (per RL-10). "Active alias" checked via `project_alias_sources`: non-borrow aliases block conservatively. DP-5 is the sole predicate that additionally requires `borrow_sources`, `project_alias_sources`, and liveness.
