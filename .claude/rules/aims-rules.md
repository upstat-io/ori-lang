...
**Demand construction**: the propagated demand always uses `Access = Owned`. This matches IA-5 step 1's unconditional Owned promotion for aggregate builder arguments — the source aggregate must be Owned because field cleanup at scope exit requires Owned access for RcDec. The locality component uses `max(source_current_locality, alias_locality)`. Cardinality uses `seq_add`. Consumption is NOT propagated.
