...
The key correctness property: `ReturnContract` extraction traces definitions STRUCTURALLY (IR instruction types and callee contracts), then VALIDATES the result against the converged `AimsStateMap`. The structural trace provides the optimistic contract; the validation downgrades it when the converged state disagrees. Specifically: if the structural trace yields `uniqueness = Unique` but the returned variable's converged state has `uniqueness ≠ Unique` (e.g., the variable escaped to a global and CN-6 demoted it), the contract is downgraded to `MaybeShared`. Similarly, if the structural trace yields `preserves_freshness = true` but the variable's converged locality is `HeapEscaping` with non-Unique uniqueness (indicating the value escaped), freshness is downgraded to `false`. This validation prevents unsound `noalias` on returned pointers that structurally trace to fresh allocations but were aliased at runtime (e.g., stored into a global before being returned).
