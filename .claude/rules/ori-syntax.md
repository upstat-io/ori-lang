...
**String methods**: `.split(sep:)`, `.trim()`, `.substring(start:, end:)`, `.to_uppercase()`, `.to_lowercase()`, `.starts_with(prefix:)`, `.ends_with(suffix:)`, `.contains(substr:)`, `.len()`, `.is_empty()`, `.as_bytes()`→`[byte]` (zero-copy), `.to_bytes()`→`[byte]` (copy), `.byte_len()`→`int`; assoc: `str.from_utf8(bytes:)`→`Result<str, Error>`, `str.from_utf8_unchecked(bytes:)`→`str` (unsafe)
