//! Decision-tree evaluation against runtime values.
//!
...
use ori_ir::canon::tree::{DecisionTree, PathInstruction, ScrutineePath, TestValue};
use ori_ir::{
    ...
    match tree {
        ...
        DecisionTree::Switch {
            ...
            for (test_value, subtree) in edges {
                if test_matches(&sub_value, test_value, interner) {
...
fn test_matches(value: &Value, test_value: &TestValue, interner: &StringInterner) -> bool {
    match test_value {
        TestValue::Tag {
            variant_index,
            variant_name,
        } => match value {
            // Evaluator variants carry their interned name, while compiled
            // backends consume the same tree's numeric discriminant.
            Value::Variant {
                variant_name: vn, ..
            } => *vn == *variant_name,
