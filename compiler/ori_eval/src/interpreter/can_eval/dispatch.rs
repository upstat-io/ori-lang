//! Stack-safe dispatch over sugar-free [`CanExpr`] nodes.

use ori_ir::canon::{
    CanBindingPatternId, CanExpr, CanId, CanNamedExprRange, CanParamRange, CanRange, CanonResult,
    ConstantId, DecisionTreeId, IndexDispatch, MonoConstBinding, MonoInstanceId,
};
...
use super::literal::{const_to_value, eval_can_duration, eval_can_size};
...
impl Interpreter<'_> {
    ...
    /// Borrows the canonical IR installed for the active expression tree.
    #[inline]
    pub(super) fn canon_ref(&self) -> &CanonResult {
        match &self.canon {
            Some(canon) => canon,
    ...
    /// Resolves a canonical node's source span from the active arena.
    #[inline]
    pub(in crate::interpreter) fn can_span(&self, can_id: CanId) -> Span {
    ...
    fn begin_can_eval(&mut self, can_id: CanId) -> CanExpr {
    ...
    fn eval_can_inner(&mut self, can_id: CanId) -> EvalResult {
        let kind = self.begin_can_eval(can_id);
        match kind {
            ...
            CanExpr::Index {
                ...
                index,
                dispatch,
            } => self.eval_can_index(can_id, receiver, index, dispatch),
            CanExpr::If {
                cond,
                then_branch,
                else_branch,
    ...
    fn eval_can_index(
        ...
        match dispatch {
            IndexDispatch::Builtin => self.eval_can_builtin_index(can_id, receiver, index),
            IndexDispatch::Deferred => self.eval_can_deferred_index(can_id, receiver, index),
            IndexDispatch::Selected(_) => self.eval_can_user_index(receiver, index),
            IndexDispatch::Error => self.eval_can_error(can_id),
        }
    }

    fn eval_can_builtin_index(
        &mut self,
        can_id: CanId,
        receiver: CanId,
        index: CanId,
        ...
        let value = self.eval_can(receiver)?;
        self.eval_can_builtin_index_value(can_id, value, index)
    }

    fn eval_can_deferred_index(
        &mut self,
        can_id: CanId,
        receiver: CanId,
        index: CanId,
        ...
        let value = self.eval_can(receiver)?;
        if super::super::operator_dispatch::is_builtin_indexable(&value) {
            self.eval_can_builtin_index_value(can_id, value, index)
        } else {
            self.eval_can_user_index_value(value, index)
        }
    }

    fn eval_can_user_index(&mut self, receiver: CanId, index: CanId) -> EvalResult {
        let value = self.eval_can(receiver)?;
        self.eval_can_user_index_value(value, index)
    }

    fn eval_can_builtin_index_value(
        &mut self,
        can_id: CanId,
        value: Value,
        index: CanId,
    ...
    fn eval_can_user_index_value(&mut self, value: Value, index: CanId) -> EvalResult {
