...
//! # Why this exists
//!
//! PC-2: no [`Tag::Var`] survives in any type-bearing position of the typed
//! IR. Without producer-side enforcement, violations would leak through to
...
//!   [`FunctionSig`] (`param_types` + `return_type`): unannotated params /
//!   returns ship out of the signatures pass as fresh [`Tag::Var`]s and
//!   MUST resolve before Bodies-group exit.
//! - Walks compound types via the canonical `Pool::visit_children` helper
//!   rather than cloning a parallel tag-dispatch ladder.
...
//!   (TF-2: flags computed once at interning time), but [`VarState::Link`]
//!   may resolve later via union-find. The `HAS_VAR` fast-path gate is only
//!   sound after link resolution at each step.
//! - Emits one diagnostic per ([`ExprIndex`], diagnostic-code) site;
//!   multiple unbound vars under one [`ExprIndex`] collapse to a single
//!   `E2005`.
//! - The `Tag::Scheme` path relies on the `PROPAGATE_MASK` propagation of
//!   `HAS_VAR` through schemes in `Pool::compute_flags`.
//!
//! # Dependencies
//!
//! - `Pool::visit_children` must be visible to this module (`pub(crate)`).
//! - The `HAS_VAR` propagation for `Tag::Scheme` (`compute_flags`) must be
//!   in place; otherwise schemes wrapping unbound-var bodies would be
//!   skipped by the top-level gate (TF-5: `HAS_VAR` fast-path).
...
/// [`AmbiguousTypeSite::LambdaParam`] span narrowing. Bundled into one
/// struct (>3 params → config struct) — the three closures always travel
/// together.
///
...
///   for specialized E2005 wording (empty lists get a `[int]`-annotation
///   hint; lambda params get a typed-lambda hint).
...
///   unresolved `Tag::Var` — the whole-lambda span is too wide. Returns
///   `None` when the `ExprIndex` does not point at a
...
///
/// The classification drives specialized E2005 wording: empty list
/// literals (both `List([])` and `ListWithSpread([])`) map to `EmptyList`;
/// `Lambda` expressions map to `LambdaParam`; everything else (including
/// empty maps/sets) falls back to `Expression`.
...
/// PC-2 ("no `Tag::Var` in any type-bearing IR position").
///
...
/// a given position emits one `E2005`, deduplicated by (Code, Span).
/// Signature positions (params + return) emit against the
...
///
/// Fast paths (TF-5: `HAS_VAR` fast-path gate):
///   - Before every [`TypeFlags::HAS_VAR`] check at every walk step, the
///     idx is resolved via [`Pool::resolve_fully`] — flag caching at intern
///     time predates [`VarState::Link`] mutation (flags are stable, vars
///     are not).
///   - Types with `!HAS_VAR` short-circuit the walk.
///   - Types with [`TypeFlags::HAS_ERROR`] short-circuit at the top-level
///     gate (TK-3 poison + ER-4 cascade suppression).
///
/// [`Tag::BoundVar`] (scheme-quantified) is silently skipped: per TF-1
/// (presence flags), `Tag::BoundVar` sets `HAS_BOUND_VAR`, not `HAS_VAR`,
/// so a scheme whose only free variables are bound has `HAS_VAR = false`
/// on its body (given `HAS_VAR` propagation through schemes per TF-3). The walker
/// recurses into scheme bodies via the canonical `Pool::visit_children`
/// helper — no parallel tag-dispatch ladder.
///
/// # Parameters
///
...
///   Bundling keeps the parameter count within the `>3 params → config
///   struct` guidance and lets callers construct the closure set once per
///   body-checking pass.
/// - `errors` — mutable accumulator; new [`TypeCheckError`] values are
///   appended (ER-1: accumulate, don't bail).
#[expect(
...
pub fn validate_body_types(
    ...
    for (expr_idx, ty) in entries {
        ...
        // For LambdaParam sites, narrow the primary diagnostic span from
        // the whole-lambda span to the specific parameter-token span that
        // carries the unresolved `Tag::Var`.
...
///
/// The lambda expression's span covers the entire `x -> body` form, but
/// the E2005 wording "cannot infer the type of this closure parameter; add
/// a full typed-lambda annotation like `(x: int) -> ReturnT = body`" is
/// actionable only against the parameter token. This helper resolves the
/// lambda's inferred type, unwraps any enclosing [`Tag::Scheme`]
/// (polymorphic lambdas carry a scheme wrapper per SC-1: scheme layout),
/// and walks the function signature's parameter types for the first one
/// containing an unresolved `Tag::Var` (via [`TypeFlags::HAS_VAR`] per
/// TF-5: `HAS_VAR` fast-path).
///
/// Returns `None` when:
...
///
/// `HAS_VAR` is the canonical fast-path gate (TF-3: propagation through
/// children, including schemes) — it is set iff an unbound `Tag::Var` is
/// reachable under the type. The helper reads it after `resolve_fully`
/// (TF-2 staleness guard: flags cached at intern time).
fn narrow_to_lambda_param_span(
    ...
    // shape per SC-1: `Scheme([var_ids], Function(...))`).
    ...
    // window (TF-2: flags cached at intern time).
...
/// at the error position. It is threaded unchanged through recursion —
/// classification is a property of the top-level expression, not of its
/// type subchildren.
fn collect_first_unbound_var(
    ...
    //     variables (TF-3 propagation through children ensures the flag is
    //     set iff an unbound Var is reachable).
    ...
    if !flags.contains(TypeFlags::HAS_VAR) {
        // !HAS_VAR ⇒ no unbound var reachable (TF-3 propagation).
        return false;
    }
...
fn check_var_tag(
    ...
    match pool.var_state(var_id) {
        ...
        // are legitimate at PC-2 exit (UN-6: rigid variable rejection).
