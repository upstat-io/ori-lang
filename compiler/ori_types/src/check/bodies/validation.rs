//! Shared finalization and validators for checked bodies.
...
#[derive(Debug)]
pub(in crate::check) struct BodyOutputs {
    ...
    /// Exact semantic route selected for each index site.
    pub index_dispatch_selections: Vec<(ExprId, crate::IndexDispatchSelection)>,
...
pub(in crate::check) fn finalize_body_and_export(
...
pub(super) fn validator_expr_id(expr_index: ExprIndex) -> ExprId {
...
fn run_drop_partial_move_validator(
    checker: &mut ModuleChecker<'_>,
    expr_types: &FxHashMap<ExprIndex, Idx>,
    body_root: ExprId,
) {
...
fn run_consumed_binding_validator(checker: &mut ModuleChecker<'_>, body_root: ExprId) {
    let validation_errors: Vec<TypeCheckError> = {
        let arena = checker.arena();
