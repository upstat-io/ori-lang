...
use crate::{
    ...
    TraitRegistry, TypeCheckError, TypeCheckWarning, TypeFlags, TypeRegistry, UnifyEngine,
    UnifyError, VarState,
};
...
impl<'pool> InferEngine<'pool> {
    ...
    /// Default `VarState::Unbound` vars reachable from empty collection
    ...
    /// from THIS body's `expr_types` entries whose AST `ExprKind` is an
    /// empty-literal form are defaulted. Cross-body pollution is structurally
    /// impossible — sibling bodies' vars never appear in this body's
    /// `expr_types`.
    ///
    ...
    ///
    /// Convenience wrapper that mutates a whole [`FunctionSig`]
    /// in-place and refreshes its Merkle hashes. Callers that only have
    /// loose `param_types` / `return_type` (e.g., `check_impl_method` which
    /// constructs its `FunctionSig` at the end via `build_method_sig`) should
    /// call [`InferEngine::default_unbound_vars_in_scope`] directly and
    /// construct the hashes themselves.
