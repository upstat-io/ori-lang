...
pub(crate) fn jit_symbol_mappings() -> Vec<(&'static str, usize)> {
    vec![
        ...
        (
            "ori_list_concat_cow",
            runtime::ori_list_concat_cow as *const () as usize,
        ),
        // Map literal construction
        (
            "ori_map_literal_alloc",
            runtime::map::ori_map_literal_alloc as *const () as usize,
        ),
        (
            "ori_map_literal_put",
            runtime::map::ori_map_literal_put as *const () as usize,
        ),
        // Iterator sources
        (
            "ori_iter_from_list",
            runtime::iterator::ori_iter_from_list as *const () as usize,
        ),
        (
            "ori_iter_from_range",
            runtime::iterator::ori_iter_from_range as *const () as usize,
        ),
        (
            "ori_iter_from_str",
            runtime::iterator::ori_iter_from_str as *const () as usize,
        ),
        (
            "ori_iter_from_map",
            runtime::iterator::ori_iter_from_map as *const () as usize,
        ),
        // Iterator core
        (
            "ori_iter_next",
            runtime::iterator::ori_iter_next as *const () as usize,
        ),
        (
            "ori_iter_drop",
            runtime::iterator::ori_iter_drop as *const () as usize,
        ),
        // Iterator adapters
        (
            "ori_iter_map",
            runtime::iterator::ori_iter_map as *const () as usize,
        ),
        (
            "ori_iter_filter",
            runtime::iterator::ori_iter_filter as *const () as usize,
        ),
        (
            "ori_iter_take",
            runtime::iterator::ori_iter_take as *const () as usize,
        ),
        (
            "ori_iter_skip",
            runtime::iterator::ori_iter_skip as *const () as usize,
        ),
        (
            "ori_iter_enumerate",
            runtime::iterator::ori_iter_enumerate as *const () as usize,
        ),
        (
            "ori_iter_zip",
            runtime::iterator::ori_iter_zip as *const () as usize,
        ),
        (
            "ori_iter_chain",
            runtime::iterator::ori_iter_chain as *const () as usize,
        ),
        // Iterator consumers
        (
            "ori_iter_collect",
            runtime::iterator::ori_iter_collect as *const () as usize,
        ),
        (
            "ori_iter_collect_set",
            runtime::iterator::ori_iter_collect_set as *const () as usize,
        ),
        (
            "ori_iter_count",
            runtime::iterator::ori_iter_count as *const () as usize,
        ),
        (
            "ori_iter_any",
            runtime::iterator::ori_iter_any as *const () as usize,
        ),
        (
            "ori_iter_all",
            runtime::iterator::ori_iter_all as *const () as usize,
        ),
        (
            "ori_iter_find",
            runtime::iterator::ori_iter_find as *const () as usize,
        ),
        (
            "ori_iter_for_each",
            runtime::iterator::ori_iter_for_each as *const () as usize,
        ),
        (
            "ori_iter_fold",
            runtime::iterator::ori_iter_fold as *const () as usize,
        ),
        // List mutation
        (
            "ori_list_push",
            runtime::ori_list_push as *const () as usize,
        ),
        (
            "ori_list_take",
            runtime::ori_list_take as *const () as usize,
        ),
        (
            "ori_list_box_new",
            runtime::ori_list_box_new as *const () as usize,
        ),
        (
            "ori_list_push_new",
            runtime::ori_list_push_new as *const () as usize,
        ),
        // List COW
        (
            "ori_list_push_cow",
            runtime::ori_list_push_cow as *const () as usize,
        ),
        (
            "ori_list_pop_cow",
            runtime::ori_list_pop_cow as *const () as usize,
        ),
        (
            "ori_list_set_cow",
            runtime::ori_list_set_cow as *const () as usize,
        ),
        (
            "ori_list_insert_cow",
            runtime::ori_list_insert_cow as *const () as usize,
        ),
        (
            "ori_list_remove_cow",
            runtime::ori_list_remove_cow as *const () as usize,
        ),
        (
            "ori_list_reverse_cow",
            runtime::ori_list_reverse_cow as *const () as usize,
        ),
        (
            "ori_list_sort_cow",
            runtime::ori_list_sort_cow as *const () as usize,
        ),
        (
            "ori_list_sort_stable_cow",
            runtime::ori_list_sort_stable_cow as *const () as usize,
        ),
        // List query
        (
            "ori_list_first",
            runtime::ori_list_first as *const () as usize,
        ),
        (
            "ori_list_last",
            runtime::ori_list_last as *const () as usize,
        ),
        (
            "ori_list_contains_int",
            runtime::ori_list_contains_int as *const () as usize,
        ),
        (
            "ori_list_contains_str",
            runtime::ori_list_contains_str as *const () as usize,
        ),
        (
            "ori_list_concat",
            runtime::ori_list_concat as *const () as usize,
        ),
        (
            "ori_list_reverse",
            runtime::ori_list_reverse as *const () as usize,
        ),
        // Map operations
        (
            "ori_map_contains_key",
            runtime::map::ori_map_contains_key as *const () as usize,
        ),
        (
            "ori_map_keys_to_list",
            runtime::map::ori_map_keys_to_list as *const () as usize,
        ),
        (
            "ori_map_values_to_list",
            runtime::map::ori_map_values_to_list as *const () as usize,
        ),
        (
            "ori_map_get",
            runtime::map::ori_map_get as *const () as usize,
        ),
        (
            "ori_map_insert_cow",
            runtime::map::cow::ori_map_insert_cow as *const () as usize,
        ),
        (
            "ori_map_remove_cow",
            runtime::map::cow::ori_map_remove_cow as *const () as usize,
        ),
        (
            "ori_map_buffer_rc_dec",
            runtime::ori_map_buffer_rc_dec as *const () as usize,
        ),
        (
            "ori_map_buffer_drop_unique",
            runtime::ori_map_buffer_drop_unique as *const () as usize,
        ),
        // Set operations
        (
            "ori_set_contains",
            runtime::set::ori_set_contains as *const () as usize,
        ),
        (
            "ori_set_insert_cow",
            runtime::set::cow::ori_set_insert_cow as *const () as usize,
        ),
        (
            "ori_set_remove_cow",
            runtime::set::cow::ori_set_remove_cow as *const () as usize,
        ),
        (
            "ori_set_union_cow",
            runtime::set::cow::ori_set_union_cow as *const () as usize,
        ),
        (
        ...
        (
            ...
            runtime::set::cow::ori_set_difference_cow as *const () as usize,
        ),
        (
            "ori_set_to_list",
            runtime::set::ori_set_to_list as *const () as usize,
        ),
        (
            "ori_set_literal_alloc",
            runtime::set::ori_set_literal_alloc as *const () as usize,
        ),
        (
            "ori_set_literal_put",
            runtime::set::ori_set_literal_put as *const () as usize,
        ),
        (
            "ori_set_buffer_rc_dec",
            runtime::ori_set_buffer_rc_dec as *const () as usize,
        ),
        (
            "ori_set_buffer_drop_unique",
            runtime::ori_set_buffer_drop_unique as *const () as usize,
        ),
        // String iteration
        (
            "ori_str_chars",
            runtime::ori_str_chars as *const () as usize,
        ),
        (
            "ori_str_split",
            runtime::ori_str_split as *const () as usize,
        ),
        // String methods
        (
            "ori_str_contains",
            runtime::ori_str_contains as *const () as usize,
        ),
        (
            "ori_str_starts_with",
            runtime::ori_str_starts_with as *const () as usize,
        ),
        (
            "ori_str_ends_with",
        ...
        (
            ...
            runtime::ori_str_to_lowercase as *const () as usize,
        ),
        (
            "ori_str_replace",
            runtime::ori_str_replace as *const () as usize,
        ),
        (
            "ori_str_repeat",
            runtime::ori_str_repeat as *const () as usize,
        ),
        // Slice operations
        (
            "ori_list_slice",
            runtime::ori_list_slice as *const () as usize,
        ),
        (
            "ori_list_slice_take",
            runtime::ori_list_slice_take as *const () as usize,
        ),
        (
            "ori_list_slice_drop",
            runtime::ori_list_slice_drop as *const () as usize,
        ),
        (
            "ori_list_materialize_slice",
            runtime::ori_list_materialize_slice as *const () as usize,
        ),
        (
            "ori_str_substring",
            runtime::ori_str_substring as *const () as usize,
        ),
        // RC utilities
        (
            "ori_rc_live_count",
            runtime::ori_rc_live_count as *const () as usize,
        ),
        (
            "ori_rc_reset_live_count",
            runtime::ori_rc_reset_live_count as *const () as usize,
        ),
        (
            "ori_rc_is_unique",
            runtime::ori_rc_is_unique as *const () as usize,
        ),
        (
            "ori_rc_is_unique_or_null",
            runtime::ori_rc_is_unique_or_null as *const () as usize,
        ),
        (
            "ori_rc_realloc",
            runtime::ori_rc_realloc as *const () as usize,
        ),
        (
            "ori_memcpy_elements",
            runtime::ori_memcpy_elements as *const () as usize,
        ),
        (
            "ori_memmove_elements",
            runtime::ori_memmove_elements as *const () as usize,
        ),
        (
            "ori_list_ensure_capacity",
            runtime::ori_list_ensure_capacity as *const () as usize,
        ),
        // Empty collection sentinels
        (
            "ori_list_empty",
            runtime::ori_list_empty as *const () as usize,
        ),
        (
            "ori_str_empty",
            runtime::ori_str_empty as *const () as usize,
        ),
        (
            "ori_map_empty",
            runtime::map::ori_map_empty as *const () as usize,
        ),
        (
            "ori_set_empty",
            runtime::set::ori_set_empty as *const () as usize,
        ),
        // Buffer reuse
        (
            "ori_list_reset_buffer",
            runtime::ori_list_reset_buffer as *const () as usize,
        ),
        // Catch recovery (Itanium EH — used by JIT catch blocks)
        (
            "ori_catch_cleanup",
            runtime::ori_catch_cleanup as *const () as usize,
        ),
        (
            "ori_catch_recover",
            runtime::ori_catch_recover as *const () as usize,
        ),
