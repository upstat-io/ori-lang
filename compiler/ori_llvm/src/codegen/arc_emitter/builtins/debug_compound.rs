...
impl<'scx: 'ctx, 'ctx> ArcIrEmitter<'_, 'scx, 'ctx, '_> {
    ...
    pub(super) fn emit_option_debug_branch(
        ...
        let none_bb_current = self.builder.current_block()?;
        ...
        let some_bb_current = self.builder.current_block()?;
        self.builder.br(merge_bb);
    ...
    pub(super) fn emit_list_debug(
        ...
        let (result, close_bb_end) = self.emit_debug_sequence_tail(
            ...
            |emitter, idx, acc| {
                ...
                let elem_ptr = emitter.builder.gep(elem_llvm_ty, data, &[idx], "ldbg.epi");
                let elem = emitter.builder.load(elem_llvm_ty, elem_ptr, "ldbg.ei");
                let elem =
                    emitter.sext_narrowed_int_element(elem, list_ty, elem_ty, "ldbg.ei.sext");
