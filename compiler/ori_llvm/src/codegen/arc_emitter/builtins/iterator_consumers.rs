...
impl<'scx: 'ctx, 'ctx> ArcIrEmitter<'_, 'scx, 'ctx, '_> {
    pub(in crate::codegen) fn emit_iter_collect(
        ...
        self.emit_rt_call(
            func_id,
            &[iter_ptr, elem_size_val, elem_inc_fn, elem_dec_fn, out_ptr],
            "",
        );
    ...
    pub(in crate::codegen) fn emit_iter_count(
        ...
        self.emit_rt_call(func_id, &[iter_ptr, elem_size_val], "iter.count")
    }
    ...
    fn emit_iter_any_all(
        ...
        Some(self.builder.icmp_ne(result, zero_i8, "iter.any_all.bool"))
    }
