//! Core state and lifecycle for the ID-based LLVM instruction builder.
...
use super::super::eh_model::EhModel;
use super::super::value_id::{BlockId, FunctionId, LLVMTypeId, ValueArena, ValueId};
use super::checked_ops;
...
pub struct IrBuilder<'scx, 'ctx> {
    ...
    pub(in crate::codegen) builder: InkwellBuilder<'ctx>,
    /// Shared LLVM context for type creation.
    pub(in crate::codegen) scx: &'scx SimpleCx<'ctx>,
    /// Arena storing all LLVM values behind IDs.
    pub(in crate::codegen) arena: ValueArena<'ctx>,
    /// Currently-active function (set by `set_current_function`).
    pub(in crate::codegen) current_function: Option<FunctionId>,
    /// Currently-active basic block (tracked for save/restore).
    pub(in crate::codegen) current_block: Option<BlockId>,
    ...
    pub(in crate::codegen) codegen_errors: Cell<u32>,
    /// Descriptive messages for each codegen error (for diagnostics).
    pub(in crate::codegen) codegen_error_descriptions: RefCell<Vec<String>>,
    ...
    pub(in crate::codegen) last_call_site: Option<CallSiteValue<'ctx>>,
    ...
    pub(super) jit_aggregate_components: FxHashMap<ValueId, Vec<Option<ValueId>>>,
    ...
    pub(super) global_strings: FxHashMap<String, ValueId>,
    ...
    pub(in crate::codegen) catch_unwind_target: Option<BlockId>,
}
...
impl<'scx, 'ctx> IrBuilder<'scx, 'ctx> {
    ...
    pub fn runtime_fn(&mut self, name: &'static str) -> FunctionId {
        if self.mode == CompilationMode::Jit {
            assert!(
                super::super::runtime_decl::runtime_functions::is_jit_allowed(name),
        ...
        let id = super::super::runtime_decl::declare_single(self, name);
    ...
    pub fn try_runtime_fn(&mut self, name: &str) -> Option<FunctionId> {
        ...
        let (static_name, id) = super::super::runtime_decl::try_declare_single(self, name)?;
        if self.mode == CompilationMode::Jit {
            assert!(
                super::super::runtime_decl::runtime_functions::is_jit_allowed(static_name),
