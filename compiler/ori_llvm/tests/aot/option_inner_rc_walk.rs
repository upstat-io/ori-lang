...
/// Build + run one cell under the gated burden probe; map the outcome to a
/// human-readable failure or `Ok`.
fn run_cell(cell: &Cell) -> Result<(), String> {
    let (exit, stdout, stderr) = compile_and_run_with_build_env(cell.source, GATED_PROBE);
    match exit {
        0 if stdout.trim() == "c" => Ok(()),
        0 => Err(format!("{}: wrong stdout {stdout:?}", cell.name)),
        2 => Err(format!("{}: leak under gated probe\n{stderr}", cell.name)),
...
/// under the gated burden probe. Self-verifying: the count assertion proves no
...
fn option_inner_field_dec_walk_tag_guarded_and_leak_free() {
    ...
    assert!(
        failures.is_empty(),
        "gated-probe Option<RC-inner> dec-walk failures:\n{}",
        failures.join("\n")
    );
}
...
/// under-emit and leak Some — caught here as a gated-probe leak (exit 2).
#[test]
fn option_str_field_some_dec_frees_inner_str() {
    ...
    assert_eq!(
        exit, 0,
        "Some(str) Option field dec must run leak-free under the gated probe\nstdout:{stdout}\nstderr:{stderr}"
    );
...
fn list_option_str_element_dec_tag_guarded_and_leak_free() {
    ...
    assert_eq!(
        exit, 0,
        "[Option<str>] element dec must run leak-free under the gated probe\nstdout:{stdout}\nstderr:{stderr}"
    );
...
/// leak-free under the gated probe.
#[test]
fn option_str_field_inc_via_clone_leak_free() {
    ...
    assert_eq!(
        exit, 0,
        "Option<str> inc-via-clone must run leak-free under the gated probe\nstdout:{stdout}\nstderr:{stderr}"
    );
