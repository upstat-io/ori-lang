...
int main(int argc, char **argv) {
    ...
    double ori_time = (double)(end - start) / CLOCKS_PER_SEC;
    double ori_per_call = (ori_time / iterations) * 1e9;
    use_value(sum);
    ...
    printf("  Ori/LLVM: %.3f sec (%.2f ns/call)\n", ori_time, ori_per_call);
    printf("  C (-O2):    %.3f sec (%.2f ns/call)\n", c_time, c_per_call);
    printf("  Ratio:      %.2fx (>1 = Ori faster)\n", c_time / ori_time);
