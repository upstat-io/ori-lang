//! COW insertion and value replacement.
...
use super::super::{
    ...
    write_map_struct, HashTableLayout, OriMap, META_OCCUPIED,
};
...
pub extern "C" fn ori_map_insert_cow(
    ...
    if let Some(bucket) = found_bucket {
        ...
        cow_insert_existing(
            data, n, c, ks, vs, bucket, value, key_inc, val_inc, key_dec, val_dec, cow_mode,
            out_ptr,
        );
    } else {
        // New key — insert into hash table
        cow_insert_new(
            data, n, c, ks, vs, key, value, hash, key_hash, key_inc, val_inc, key_dec, val_dec,
            cow_mode, out_ptr,
        );
    }
}
...
pub(crate) fn ori_map_update_cow(
    ...
    cow_insert_existing(
        data, n, c, ks, vs, bucket, new_value, key_inc, val_inc, key_dec, val_dec, cow_mode,
        out_ptr,
    );
}
