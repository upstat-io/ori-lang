...
@test_should_fail tests _ () -> void = print(msg: "test");
"#,
    );
...
@test_expect_failure tests _ () -> void = panic(msg: "expected failure");
"#,
    );
...
fn test_parse_derive_attribute() {
    let (result, _interner) = parse_with_errors(
        r#"
#[derive(Eq, Clone)]
@test_with_derive tests _ () -> void = print(msg: "test");
"#,
    );
...
fn test_parse_unknown_attribute() {
    let (result, _interner) = parse_with_errors(
        r#"
#[unknown("value")]
@test_unknown tests _ () -> void = print(msg: "test");
"#,
    );
...
fn test_parse_attribute_missing_paren() {
    let (result, _interner) = parse_with_errors(
        r"
#[skip]
@test_bad tests _ () -> void = assert(cond: true);
",
    );
...
#[skip("reason")]
#[fail("expected")]
@test_multi tests _ () -> void = print(msg: "test");
"#,
    );
...
@test_extended tests _ () -> void = print(msg: "test");
"#,
    );
