...
//!
//! Per `plans/aims-proofing-suite/section-11-coexistence-handshake-proofs.md`
//! §11.1 success_criterion 8 (compiler-conformance cross-walk Tier 2): drives
...
//! `eliminate_burden_ops` that the §11 calculus models) has not yet landed in
//! `compiler_repo/compiler/ori_arc/src/aims/`. CH-1 / CH-2 / CH-3 / CH-5 /
//! CH-comp therefore map to `conformance: target-only` per the §11.1 Decision
//! row; only CH-4 has a shipped emission site (`eliminate_burden_ops`) the
//! behavioral harness can drive.
//!
//! The behavioral fixtures below are stubbed pending the §04A.2 production
//! coexistence_dispatch surface landing (per
...
///
/// Stub-pending the §04A.2 production coexistence_dispatch surface. When the
...
fn ch4_p1_aimsstatemap_immutability_under_burden_elim() {
    // Pending §04A.2 production coexistence_dispatch surface.
    //
    ...
    //
    // The current §04A.2 design ships `eliminate_burden_ops` as `pub(crate)`;
    // the behavioral fixture awaits the production composition surface
    // (per `plans/aims-proofing-suite/section-11-coexistence-handshake-proofs.md`
    // §11.R Third Party Review Findings + the §04A.2 shipping schedule).
}
