...
pub(crate) fn emit_invoke_unwind_pair_net_releases(
    ...
    for (block_idx, block) in func.blocks.iter().enumerate() {
        ...
        for (_, succ, var) in compute_invoke_edge_dead_set(env, block_idx) {
            ...
            if !super::carries_burden(func, var) {
                continue;
            }
            if super::is_owned_transfer_arg_at_terminator(func, block_idx, var) {
                continue;
            }
            if !pred_pair_nets_zero(func, block_idx, var) {
                continue;
            }
            if super::has_whole_var_burden_dec_in_block(func, succ, var) {
                continue;
            }
