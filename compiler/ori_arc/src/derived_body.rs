...
/// method-call provenance fact for the shared target-rewrite seam. AIMS remains
...
mod tests {
    ...
    use crate::{
        build_derived_clone_identity, build_derived_eq, ArcInstr, ArcTerminator, ArcValue,
        ArcVarId, DerivedCloneBodyError, DerivedEqBodyError, MethodCallForm, Ownership, PrimOp,
    ...
    fn eq_body_or_panic(
        ...
        match build_derived_eq(EXECUTABLE, signature, pool) {
            Ok(body) => body,
            Err(error) => panic!("{expectation}: {error}"),
        }
    }
    ...
    fn generic_eq_signature_is_rejected() {
        ...
        assert_eq!(
            build_derived_eq(EXECUTABLE, &generic, &pool),
    ...
    fn wrong_eq_arity_is_rejected() {
        ...
        assert_eq!(
            build_derived_eq(EXECUTABLE, &missing, &pool),
        ...
        assert_eq!(
            build_derived_eq(EXECUTABLE, &extra, &pool),
            Err(DerivedEqBodyError::InvalidParameterShape {
                parameter_names: 3,
                parameter_types: 3,
            })
