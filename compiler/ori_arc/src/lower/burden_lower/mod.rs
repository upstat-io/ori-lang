...
/// `ORI_DISABLE_LOOP_CARRIED_DEAD_COLLECTION_PARAM_RELEASE=1` skips the Phase-5
/// RL-5 dead-at-entry treatment for a fresh `ori_list_take` (for-yield collect)
/// collection threaded through a loop and dead at the post-loop block-param
/// ([`ownership_scans::compute_loop_carried_dead_collection_param_lineage`]) — the
/// collection analog of `LOOP_CLOSURE_DEAD_PARAM_RELEASE_DISABLED` (BUG-04-238).
/// Default (unset): the whole same-alloc lineage (the `ori_list_take` root +
/// `Let`-Var aliases + `Jump`-arg-threaded loop block-params) is removed from
/// `owned_vars_needing_rc` (suppressing the net-0 keep-alive pairs) and EXACTLY
/// ONE whole-var `BurdenDec` is placed at the dead post-loop block-param entry.
/// With the toggle set, the base walk's net-0 keep-alive pairs return — the dead
/// loop-carried collect leaks its allocation (the `let a = for .. yield ..; for ..
/// yield ..` shape, exit 2). Bisection surface: isolates a loop-carried-dead-
/// collection leak to this treatment vs the rest of the Phase-5 walk. Spec: Annex
/// E §AIMS RL-5 (`RL5_dead_at_entry_cleanup` + `RL5_cleanup_balanced`) + RL-2.
static LOOP_CARRIED_DEAD_COLLECTION_PARAM_RELEASE_DISABLED: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("ORI_DISABLE_LOOP_CARRIED_DEAD_COLLECTION_PARAM_RELEASE").as_deref() == Ok("1")
});
