...
pub(super) fn emit_burden_ops_for_blocks(
    ...
    for (block_idx, block) in func.blocks.iter_mut().enumerate() {
        ...
        // The burden walk is the sole RC emitter: it MUST emit the
        // borrowed-Invoke-arg scope-exit dec itself. The empty borrowed-arg set
        // un-suppresses those decs.
        let terminator_borrowed_args: FxHashSet<ArcVarId> = FxHashSet::default();
...
fn emit_instr_burdens(new_body: &mut Vec<ArcInstr>, instr: ArcInstr, ctx: &BurdenEmitCtx<'_>) {
    ...
    // The burden walk is the sole RC emitter: the surviving whole-var burden ops
    // lower mechanically to real `RcInc`/`RcDec`, so a `BurdenInc dst` placed
    // BEFORE the defining `Construct` / `PartialApply` / `Reuse` /
    // `Apply`-result / dup-`Let` would lower to an `RcInc` on an undefined var
    // (the ArcIrEmitter rejects it) — emit the Inc AFTER the instruction so the
    // lowered RcInc sees a defined value.
    ...
    // Emit the FRESH-site Inc AFTER the defining instruction so the lowered
    // RcInc references a defined dst.
    new_body.push(instr);
    ...
    emit_last_use_decs(new_body, ctx);
}
...
fn emit_last_use_decs(new_body: &mut Vec<ArcInstr>, ctx: &BurdenEmitCtx<'_>) {
    ...
    for &var in last_use_vars {
        if let Some(scan) = last_use_dec_suppressor(ctx, var) {
...
fn last_use_dec_suppressor(ctx: &BurdenEmitCtx<'_>, var: ArcVarId) -> Option<&'static str> {
    // The burden walk is the sole RC emitter: a value consumed at an owned
    // position whose callee BORROWS (no real transfer) is released by the burden
    // path — the ApplyIndirect closure-receiver + borrowed-Apply-arg case — so
    // no instruction-local transfer-suppression is applied (the dec is emitted).
...
fn emit_terminator_burden_decs(
    ...
    if let Some(last_use_vars) = analysis.last_uses_at.get(&(block_idx, terminator_idx)) {
        for &var in last_use_vars {
            ...
            if analysis.transfer_through_return_param_vars.contains(&var) {
                continue;
            }
