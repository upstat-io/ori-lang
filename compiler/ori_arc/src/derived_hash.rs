...
fn emit_enum_hash(
    ...
    for (variant_index, (_, fields)) in variants.into_iter().enumerate() {
        let switch_value =
            u64::try_from(variant_index).map_err(|_| DerivedHashBodyError::IndexOverflow {
                receiver_type,
                index_kind: "variant",
                index: variant_index,
            })?;
...
fn emit_field_hash_fold(
    ...
    for (field_index, field_type) in fields.iter().copied().enumerate() {
        ...
        let offset =
            u32::try_from(field_index).map_err(|_| DerivedHashBodyError::IndexOverflow {
                receiver_type,
                index_kind: "field",
                index: field_index,
            })?;
...
fn map_type_error(error: ConcreteTypeError) -> DerivedHashBodyError {
    match error {
        ConcreteTypeError::InvalidTypeIndex { position, ty } => {
            DerivedHashBodyError::InvalidTypeIndex { position, ty }
        }
        ConcreteTypeError::NonConcreteType { position, ty } => {
            DerivedHashBodyError::NonConcreteType { position, ty }
        }
    }
}
