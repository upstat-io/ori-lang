...
**What the amendment buys, and when.** Moving the cache out of `build/cache/` delivers **bounded, self-evicting storage under one owner** immediately; that alone satisfies T4. Cross-project *sharing* is the second benefit, and it is **in scope here** — D1a below is part of this proposal's delivered work, not sequenced follow-on. One honest timing note: the *motivating* benefit — reusing a shared dependency compiled once machine-wide — needs multi-file caching, because `has_unfingerprinted_imports` bypasses the cache for any source carrying `use` imports, so the shipped cache is single-file-only today. A narrower case shares immediately: two distinct projects each holding a single import-free `.ori` file with byte-identical source already hit one entry under D1a, since `has_unfingerprinted_imports` scopes only on imports, not on project identity. Sharing is therefore not fully dormant pre-multi-file — it is limited to import-free single-file builds. D1a therefore lands *ready*, and the shared-dependency duplication Alternative 1 rejects becomes reachable the moment multi-file caching does. Landing the keying change with the amendment rather than after it means the approved decision is overturned once, with both benefits in the approved scope, instead of amended now and amended again later.
...
- **Diagnostic rendering — REQUIRED, lands with the keying change (DWARF/CodeView surface only).** A compiler diagnostic or debugger view resolved against a shared object renders the **consuming** project's path. Stored paths are root-relative after normalization, and this surface *does* re-anchor — a debugger and the compiler's own diagnostic renderer both have the consuming project's root available (at link time and at compile time respectively). A user must never see another project's directory in their own diagnostic; if that appears, the normalization is incomplete and the cache entry is not shareable. (The runtime trace surface below is different — it does not re-anchor, and does not need to.)
- **Runtime trace paths — REQUIRED, and a SECOND path surface independent of DWARF that needs NO re-anchoring.** Source paths reach a shared object through more than debug info. `current_trace_location` reads `dc.source_path` and `emit_trace_injection` bakes it as a literal `OriStr` runtime constant into `TraceEntry.file`, which user code reads via `.trace()` / `.trace_entries()` on any traced `Error`. A shared object therefore carries the *producing* project's path in its runtime trace strings even if its DWARF is perfectly normalized. The fix here is **normalization alone, at injection time** — `emit_trace_injection` stores the root-relative form (`src/main.ori`) into `TraceEntry.file` instead of the absolute path. Unlike DWARF, this surface is **not** re-anchored: `_ori_format_error_trace` (`ori_rt/src/io/trace.rs`) renders `entry.file.as_str()` literally, is called by arbitrary user code long after linking, and has no consuming-project-root parameter — nor should it acquire one. A root-relative trace path is the correct terminal form: it is project-neutral, identical whichever project consumes the shared object, and leaks nothing. DWARF re-anchors because a debugger wants the real on-disk path; a stack trace does not. Normalizing DWARF while leaving `TraceEntry.file` absolute would leak the producing path at the first `.trace()` call — a surface DWARF normalization never touches.
- **Negative gate — over EVERY project-path surface, not DWARF alone.** Path normalization is the *precondition* for dropping the path fields, and it is not satisfied until every surface that embeds a source path into a shared object is normalized: DWARF/CodeView debug info, runtime `TraceEntry.file` injection, and any future path-bearing embedding. An implementation that removes `source-spelling` and `canonical-source` while any one surface still embeds an absolute or project-rooted path is a wrong-artifact bug, not a partial delivery — the entry becomes shareable while its contents are still project-specific.
...
| **macOS (D1b)** | **Dynamic (per the shipped build)** | **Enumerate loaded images via `_dyld_image_count()` / `_dyld_get_image_name()`, select basenames starting with `libLLVM`, and fold the identical three fields (path, size, sha256) under the identical Linux domain. Finding no `libLLVM` image is an `Err` (bypass), NOT an inference of static linkage.** |

- **macOS links LLVM dynamically, always, under the shipped build.** `ori_llvm/Cargo.toml` sets `inkwell`'s `llvm21-1-force-dynamic` and `llvm-sys`'s `force-dynamic` for `cfg(not(windows))` — which includes macOS. There is no build configuration in which a macOS `oric` statically links LLVM, so macOS mirrors **Linux's** dynamic arm, not Windows's static one. The earlier "Either / infer static when no image is found" framing was wrong: since the image is always present, its absence is evidence of an *enumeration failure*, and folding a fixed static constant there would treat two different LLVM builds as identical — the wrong-artifact class this proposal exists to prevent.
- **It is a port of the Linux arm, not a new design.** The identity captured (loaded `libLLVM` by path, size, digest) is unchanged; only the enumeration mechanism differs — `/proc/self/maps` has no macOS equivalent, and `dyld` is its direct analogue. Reusing the Linux domain string is deliberate: the same LLVM produces the same digest regardless of which OS enumerated it.
- **Absence of the image is `Err`, never a static fallback.** If no `libLLVM` image is found, the arm returns `Err` and the build proceeds uncached with a diagnostic — exactly today's behavior — because under the shipped build that state can only mean the enumeration went wrong, not that LLVM is static. The safety property (an unfingerprintable toolchain must not be cached against) is preserved; only the *unconditional* macOS disablement is removed.
- **If the build config ever adds static macOS linkage**, linkage must be selected from explicit build metadata (a `cfg`/feature the compiler knows at build time), never inferred at runtime from a missing image. That is a future concern, out of scope while the shipped build is dynamic-only.
- **Enumeration runs at a quiescent point.** `libLLVM` is a normal dynamic dependency (not `dlopen`ed), so `dyld` loads it before `main()`, and the single-threaded fingerprinting step runs with the image list already settled — there is no window in which `libLLVM` transitions from unloaded to loaded during enumeration. The arm assumes this quiescence explicitly rather than polling defensively; if a future call site loses it, `_dyld_register_func_for_add_image` (Apple's callback API) replaces the count-then-index loop.
...
| Measured profile data (`test-driven-pgo`) | **No — a third class; expensive-to-regenerate** | Reproducible only by re-executing tests, not from a cache key. Retained under budget, reclaimed at **rank 6** of the global order (`binary-generation-lifecycle-proposal.md` D4) — after every regenerable class, before the RECENT final-binary tier (rank 7) — and never by the recency floor alone |
