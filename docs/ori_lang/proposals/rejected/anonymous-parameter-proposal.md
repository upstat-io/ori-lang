...
Ori requires named arguments for self-documentation:
...
Swift uses `func double(_ n: Int)` where `_` is the external name and `n` is internal. Ori could adopt this:
...
- `docs/ori_lang/v2026/spec/08-declarations.md` — Document syntax
...
The feature provides an escape hatch for verbosity while preserving Ori's default of self-documenting named arguments.
