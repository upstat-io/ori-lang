...
The `.property:` ori creates a visual rail when stacked:
...
This change aligns with Ori's core philosophy:
...
4. **Ori philosophy** — The `.` ori earns its place by creating scannable structure
...
- Reinforces the value of the `.name:` ori (creates visual structure)
