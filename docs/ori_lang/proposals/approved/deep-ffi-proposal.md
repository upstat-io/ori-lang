...
> **Errata (added 2026-04-20) — Superseded by `ffi-boundary-safety-proposal.md`:** The original rejection of Zig-style namespace header import still stands — that design hides the boundary. However, a third design exists that neither the original proposal nor the rejection considered: a block-level `#header("file.h")` attribute on `extern "c" { ... }` blocks with an **explicit function list**. The successor proposal (`ffi-boundary-safety-proposal.md`, approved 2026-04-20, §2) adopts this design. It preserves boundary visibility (function list is still required, block declaration is still required, ownership/error/free annotations are still explicit) AND eliminates the signature-boilerplate pain this FAQ entry admitted remained. The "Alternative considered: `ori bindgen header.h`" is also updated by the successor: `ori bindgen` is explicitly NOT a roadmap item — `#header(...)` is the one canonical mechanism, and `ori bindgen` would only be revisited if a libclang-less build environment becomes a supported Ori target.
