...
The provenance-partition ledger is the machine-checked foundation for reference-count placement over value provenance. The emission pass that consumes it is a target subsystem (§11); the underlying calculus — six theorem families and their composition extensions — is proven. Two objects define the model:
