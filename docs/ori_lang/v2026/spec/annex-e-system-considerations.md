...
6. **Per-field release decomposition.** A container release decomposed per named owned field shall derive its skip set solely from the union of typed field-move authorities. An authority shall come either from a local extracted view with an ownership-transferring terminal use, or from a validated frozen call contract whose inward-owner demand is exactly one projected field. In the boundary case the aggregate argument remains a read because the caller retains the shell and all other fields. A contradictory whole-value and projected-field demand shall fail closed and create no authority. The authority union is the unique clause-preserving skip set: failing to skip an authorized field nets negative one (double release), while skipping an unauthorized field nets positive one (leak); duplicate evidence is idempotent. The container's own class shall book the same single whole-value consume as an undecomposed release, and disjoint classes shall remain unchanged. The verdict refines per release site: boundary authority begins at the transferring call, flows to both normal and unwind successors of an invocation, and is absent from bypass and pre-call paths. A field shall be skipped only when every path through that release site carries its authority; mixed or cyclic uncertainty shall decline.
