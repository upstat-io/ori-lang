---
title: "Capabilities"
description: "Clause 20: Ori Language Specification — Capabilities"
order: 20
section: "Language"
---
...
> **Grammar:** See [grammar.ebnf](grammar.md) § DECLARATIONS (uses_clause), EXPRESSIONS (with_expr)
...
See [Concurrency Model](22-concurrency-model.md) for task definitions, suspending context semantics, and suspension points.
...
> **Grammar:** See [grammar.ebnf](grammar.md) § EXPRESSIONS (handler_expr)
>
...
Used by the `cache` pattern (see [Patterns § cache](15-patterns.md#1551-cache)).
...
The `handler(state: expr) { ... }` construct creates a stateful handler frame with local mutable state threaded through operations. State is frame-local and does not violate value semantics. See [Stateful Handlers](#205-stateful-handlers) for the full specification.
...
See [FFI § Unsafe Expressions](26-ffi.md#265-unsafe-expressions) for the full specification of unsafe operations.
...
> **Grammar:** See [grammar.ebnf](grammar.md) § DECLARATIONS (capset_decl)
