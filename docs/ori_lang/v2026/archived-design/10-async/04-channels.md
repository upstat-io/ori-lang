...
This document covers Ori's channel system: typed channels for communication, send/receive operations, bounded buffers, and why Ori has no shared mutable state.
...
Ori requires explicit buffer sizes:
...
Ori has no `Mutex`, `Lock`, or shared mutable variables:
