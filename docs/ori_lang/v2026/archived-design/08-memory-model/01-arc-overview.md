...
This document covers Ori's memory management strategy: pure Automatic Reference Counting with compile-time cycle prevention.

---

## Overview

Ori uses Automatic Reference Counting (ARC) as its primary memory management strategy. This provides:
...
Garbage collection (GC) is the default choice for many modern languages (Java, Go, JavaScript). However, GC has drawbacks for Ori's goals:
...
### Ori's Solution: Compile-Time Cycle Prevention

Rather than a runtime cycle collector, Ori prevents cycles at compile time. This gives us:
...
4. **Unnecessary** — Ori's functional design makes cycles naturally rare
...
### Why This Works for Ori

Ori's design makes cycles naturally impossible in most code:
...
The Ori compiler applies several optimizations:
...
Swift uses ARC similarly to Ori:
...
**Key difference:** Swift requires manual `weak` and `unowned` annotations to break cycles. Ori prevents cycles at compile time — no manual annotations, no runtime cycle collector.
...
**Key difference:** Ori prevents cycles at compile time (no GC), is compiled with static typing, and enables better optimizations.

### Objective-C

Objective-C (with ARC enabled) pioneered compiler-managed reference counting:

| Feature | Objective-C | Ori |
|---------|-------------|-------|
| Core strategy | ARC | ARC |
...
**Key difference:** Ori prevents cycles at compile time (no weak references needed), uses static typing, and uses `Option<T>` instead of nullable types.
