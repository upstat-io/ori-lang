...
This section covers Ori's capability system for managing side effects, enabling testable code, and tracking async behavior.

---

## Overview

Capabilities are Ori's solution to three related tensions:
...
@get_user (id: str) -> Result<User, Error> = try(
    // Side effect!
    let json = http.get("https://api.com/users/" + id)?,
    Ok(parse(json)),
)
```
...
```ori
@main () -> void = run(
    // ERROR: Http capability not provided
    get_user("1"),
)
```
