...
Definitions of terms used in Ori documentation.
...
Memory management strategy where the runtime tracks how many references exist to each value. When the count reaches zero, the value is deallocated. Ori uses ARC for heap-allocated values.
...
### Binding
A name associated with a value. In Ori, bindings are immutable by default. Created with `let name = value` inside `run`/`try` blocks or via function parameters.
...
### Closure
A function that captures variables from its enclosing scope. In Ori, closures capture by value (copy).

### Config Variable
A global constant defined with `$` prefix. Used for configuration values that should be easily changeable.

### Compositional Model
Ori's type system design where types are exact (no subtyping) and behavior is shared through traits (no inheritance).
...
### Expression
Code that produces a value. In Ori, almost everything is an expression, including `if/else` and `match`.
...
### Test
A function that verifies behavior, declared with `@test_name tests @target_function`. Mandatory in Ori—every function requires at least one test.

### Trait
A collection of method signatures that types can implement. Ori's mechanism for polymorphism without inheritance.
