...
In Ori, **every function must have at least one test**. This is not a guideline or best practice---it's a compiler requirement. Code without tests fails to compile.
...
This is one of Ori's most distinctive features and is central to its AI-first design philosophy.
...
Ori's approach: **force AI to prove its work**.
...
Ori eliminates this category of debt. There is no "later"---code without tests doesn't compile.
...
```
src/
  math.ori              # Contains @add
  _test/
    math.test.ori       # Contains @test_add tests @add
```
...
```
src/
  math.ori              # Contains @helper (private)
  _test/
    math.test.ori       # Has access to @helper via test import
```
...
The `_test/` convention signals "these are tests for the parent module." The compiler treats `_test/*.test.ori` files as part of the same compilation unit as the parent module, granting access to private items. This keeps implementation details hidden from external consumers while allowing thorough testing.
...
**Note:** This access only applies to `.test.ori` files in the `_test/` subdirectory of the module being tested. Regular modules in `_test/` (not ending in `.test.ori`) follow normal visibility rules.
...
```bash
ori check math.ori --format json
```
...
Rust has excellent testing culture but doesn't require tests. Ori goes further: **no tests = no compilation**.
...
No. There is no `--no-tests` flag. The requirement is fundamental to Ori's design, not a lint rule that can be silenced.
