...
**Fast path (unique):** Overwrites `elem_size` bytes at `data + index * elem_size`. The shared AIMS plan fixes the old element's logical release and cleanup obligations before physical execution; VM and compiled adapters independently encode and validate their selected mechanisms. A decrement is specific to a counter-based plan.
...
The fast path is the common case. AIMS proves that a collection reaches a mutation point with exactly one live logical owner and no conflicting borrow or sharing obligation.

The current compiled ARC projection encodes that state as a reference count of one; a VM or another compiled plan may use a different validated sharing observation without changing the AIMS fact.
