//! Shipped-lattice probe for §17 locality shipped-conformance.
//!
...
fn probe_locality_variant_count(path: &Path) -> usize {
    ...
    panic!(
        "probe: `enum Locality` not found in {}; \
         shipped lattice surface drifted away from the §17 probe contract",
        path.display()
    );
}
...
fn probe_may_escape_is_stored(path: &Path) -> bool {
    ...
    panic!(
        "probe: `struct ParamContract` not found in {}; \
         shipped contract surface drifted away from the §17 probe contract",
        path.display()
    );
}
