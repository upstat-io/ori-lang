...
status: not-started
goal: "Write comprehensive cross-product tests: 8 type categories x 8 language features x 3 execution modes"
depends_on: ["03"]
reviewed: false
third_party_review:
  status: none
...
sections:
  - id: "04.1"
    title: "Type Categories (T1-T8)"
    status: not-started
  - id: "04.2"
    title: "Language Features (F1-F8)"
    status: not-started
  - id: "04.3"
    title: "Execution Modes (M1-M3)"
    status: not-started
  - id: "04.4"
...
<!-- reviewed: hygiene fix — warning about file size -->
...
- [ ] **T9: `Set<str>`** — set with string elements (set iteration path, shares list iter but uses hash table layout) <!-- reviewed: completeness fix — sets are a distinct collection type with different buffer cleanup -->

---

## 04.2 Language Features (F1-F8)
...
### F9: Slice Iteration <!-- reviewed: completeness fix — slices share the buffer but have different cap encoding -->
...
### F10: For-Yield Producing Fat Pointers <!-- reviewed: completeness fix — yield produces a NEW list that also needs elem_dec_fn -->
...
## 04.3 Execution Modes (M1-M3)

Each test runs in three modes. The AOT test (`assert_aot_success`) implicitly covers M1 + M2.

### M1: Correctness (exit code 0)

- [ ] All F1-F8 tests return correct results (exit code 0)

### M2: Leak Detection (`ORI_CHECK_LEAKS=1`)

- [ ] All F1-F8 tests report zero leaks (`assert_aot_success` already enables `ORI_CHECK_LEAKS=1`)
...
### M4: Release Build <!-- reviewed: completeness fix — debug and release can differ due to FastISel -->
...
- [ ] At least 40 AOT tests in `fat_ptr_iter.rs` covering T1-T9 × F1-F10 matrix
...
- [ ] All tests pass in release build (`cargo b --release && timeout 150 cargo test -p ori_llvm --test aot`) <!-- reviewed: completeness fix — release build verification -->
