...
4. **`body_type_map` / `TypeInfoStore` cache poisoning across substitution scopes** (added 2026-04-18 per `/tp-help` blind-spots — codex + gemini independent flag): the local-mono and imported-mono pipelines build *separate* `body_type_map` substitution maps (`compiler/ori_types/src/infer/expr/calls/monomorphization.rs:94-107` for local; `compiler/oric/src/test/runner/llvm_backend.rs:317-355` for imported). `TypeInfoStore` (`compiler/ori_llvm/src/codegen/type_info/store.rs:37-65`) is a lazy `Idx → TypeInfo` cache + error counter shared process-wide; nounwind analyze (`compiler/ori_llvm/src/codegen/function_compiler/nounwind/analyze.rs`) calls `is_callee_intercepted → TypeInfoStore::get()`, which can cache `TypeInfo::Error` for `BoundVar`/`Var` indices that are valid in a *different* mono context. Pool-level Idx dedup means the first mono's `Idx(N)` is shared across functions — first-context error poisoning surfaces in every later context that reuses `Idx(N)`. **`TypeInfoStore` is the SURFACE point, not the root cause**: `Idx(241)` shows up there, but is produced upstream (typeck PC-2 leak per §08.1.5, ARC-lower lambda substitution at `compiler/ori_arc/src/lower/calls/lambda.rs:48-80`, or lambda_mono ARC-IR mutation at `compiler/ori_llvm/src/codegen/function_compiler/lambda_mono/mod.rs:33-129`).
...
- [ ] **Disable `fallback_bound_vars_to_int` for the new tests' code path** (`lambda_mono/type_resolve.rs:392-408`): per blind-spots.json, this fallback "silently converts unresolved-type bugs into ABI/RC bugs". Either remove it entirely (preferred per CLAUDE.md §The One Rule — workarounds banned), or gate it behind `#[cfg(debug_assertions)]` with a hard panic so the bug surfaces correctly during debugging. Replacing it with a compile-time error (`E5xxx` codegen-range diagnostic) is the architecturally-correct fix.
...
**Note on `TypeInfoStore` size (audit BLOAT_RISK)**: `compiler/ori_llvm/src/codegen/type_info/store.rs` is 388 lines (audit minor finding). Per blind-spots.json, this file is the SURFACE point where `Idx(241)` shows up, NOT the root cause. Refactoring it for size is a separate concern from the §08 mission. **Defer to a separate cleanup**: file `/add-bug` titled `"BLOAT: ori_llvm::codegen::type_info::store.rs at 388 lines (approaching 500-line limit) — split into submodules"` with subsystem `ori-codegen`, severity `low`. This deferral is anchored — the bug entry IS the concrete tracking artifact per CLAUDE.md §"Future Improvement" MUST Be Concretely Tracked. The §08.3 fix MAY add lines to this file; if the file crosses 500 lines as a result of §08.3, the bug is escalated to `medium` and the split happens before §08.5.
