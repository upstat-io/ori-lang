...
6. For each remapped var_id, REBUILD a destination-local `VarState` from the source's variant — do NOT blank-init to `Unbound`, and do NOT perform a literal byte-level clone (a literal clone preserves pool-local references inside variant payloads, which defeats the remap). Variant-by-variant rebuild rule: for `VarState::Unbound { rank }` / `VarState::Rigid { name, rank }` / `VarState::Generalized { rank }` the payload is pool-independent (ranks are scope-local to the originating InferEngine but not tied to pool indices; names are global `Name` interns), so a literal clone IS correct for those three variants. For `VarState::Link(src_idx)`, the Link target Idx is pool-local to the SOURCE pool — rebuild as `VarState::Link(dst_idx)` where `dst_idx = cache.get(&src_idx)` from the re-intern type cache (the Link target type is re-interned earlier in the recursive traversal; the cache entry MUST exist, panic via `expect` if absent). `unify/substitute.rs:78-83` and `pool/substitute/mod.rs:82-88` branch on `VarState::Generalized`; wiping it to `Unbound` distorts generic behavior. Preserving the variant with a variant-aware rebuild fixes the aliasing without introducing a new source-pool-reference leak.
...
- [ ] **6. Rebuild destination-local `VarState` from source variant (do NOT literal-clone, do NOT blank-init to `Unbound`)**: after allocating the new var_id via `target.next_var_id`, construct `target.var_states[new_id]` variant-by-variant from `source.var_states[src_id]`. Rule: for `VarState::Unbound { rank }` / `VarState::Rigid { name, rank }` / `VarState::Generalized { rank }` a literal clone is correct (rank/name payloads are pool-independent). For `VarState::Link(src_idx)` the Idx payload is source-pool-local — rebuild as `VarState::Link(cache.get(&src_idx).expect("Link target was re-interned earlier in the traversal"))` so the destination Link points into the target pool, not the source. A byte-level clone across all variants leaks source-pool references via `Link` and reintroduces the collision the remap is meant to eliminate; blanking to `Unbound` destroys `Generalized`/`Rigid` semantic state and distorts generic dispatch at every downstream substitution site (`unify/substitute.rs:78-83`, `pool/substitute/mod.rs:82-88`). Add unit tests in `pool/re_intern/tests.rs` exercising all four variants: (a) `Tag::Var(Generalized)` preserves `Generalized` in target; (b) `Tag::Var(Rigid)` preserves `Rigid` name + rank; (c) `VarState::Link(src_idx)` remaps via cache so target's Link target is a target-pool-local Idx; (d) literal-clone-only variant in the negative pin shows the Link aliasing regression.
...
- [ ] **Check roadmap §21A status**: read `plans/roadmap/section-21A-llvm.md` — subsection list is at frontmatter lines 28-45 (21.7 at :28-30, 21.11 at :40-42, 21.12 at :43-45), each currently carrying `status: in-progress`; the "verified 2026-03-29" marker is `last_verified: "2026-03-29"` on the WHOLE plan frontmatter (line 6), NOT per-subsection. The contradicted "IMPLEMENTED (verified 2026-03-29)" claim for generic monomorphization lives in the Import Resolution section at line 106. Confirm no §21A author has reopened 21.7/21.11/21.12 since 2026-03-29 and no new verification pass has landed.
...
- [ ] **Release-build parity** — run `timeout 150 cargo build --profile release-lto` then execute the §08.2 AOT corpus against the release-LTO binary; confirm `test_poly_lambda_with_imported_assert_eq_{int,str}` pass in release mode (FastISel differs from the debug path per `CLAUDE.md §Fix Completeness`, and the §08.3 pool-crate changes must hold under both)
- [ ] **Semantic pin verification (section-level)** — execute §08.3's "Run §08.2 negative pin" item as a section-close gate: `git stash` the §08.3 implementation, confirm `cargo st tests/spec/expressions/poly_lambda_with_imported_generic.ori` + `cargo test -p ori_llvm --test aot poly_lambda_mono` + `cargo test -p ori_types pool::re_intern` ALL fail with §08.1.R symptoms, then `git stash pop` and confirm all three pass again. Records that the fix (not an unrelated tree change) is what makes the tests green (per `CLAUDE.md §Fix Completeness` semantic+negative pin requirement).
