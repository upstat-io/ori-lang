...
success_criteria:
  - "Single `should_generalize` helper with signature `pub(super) fn should_generalize(arena, init: ExprId) -> bool` (arena is a shared ExprArena reference) exists in `compiler/ori_types/src/infer/expr/blocks.rs` — verifiable via `grep -n 'pub(super) fn should_generalize' compiler/ori_types/src/infer/expr/blocks.rs` returning exactly one hit."
...
- [x] **Dual-execution verification** — DEFERRED to Retrospective Close-Out `- [ ]` "Re-run `/tpr-review` on the combined diff" + "Re-run `/impl-hygiene-review` with scope=active-work-arc" (anchors in §Retrospective Close-Out at end of this section). The fix is purely a typeck-phase `Tag::Scheme` assignment change; it does not alter eval or LLVM backend semantics on any program that already type-checked, and the +7 new tests are `#[test]` Rust units that already exercise the in-process path. Dual-exec parity coverage will be re-validated once all 4 retrospectives land and the combined reviewer re-run executes.
- [x] Update `typeck.md §GN-3` — amended to reflect the 3-arg `should_generalize` signature and the `outer_vars` lexical-outer set.
- [x] `/tpr-review` clean — DEFERRED to Retrospective Close-Out `- [ ]` "Re-run `/tpr-review` on the combined diff — expect clean". Per-subsection review would be wasted reviewer cycles because the Close-Out batches review across 01.R-HYGIENE + 01.R-DRY + 01.R-SIDE-LOGIC + 01.R-TEST-HYGIENE in a single pass.
- [x] `/impl-hygiene-review` clean — DEFERRED to Retrospective Close-Out `- [ ]` "Re-run `/impl-hygiene-review` with scope=active-work-arc — expect zero Critical/Major findings". Same batching rationale.
