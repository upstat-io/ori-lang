...
- [x] `.claude/hooks/block-banned-commands.sh` denies `gemini` invocations with `timeout: 60000` (under the 1200000ms / 20-minute minimum) and `timeout: 3600000` (over the 2100000ms / 35-minute maximum). Satisfies mission criterion: "`.claude/hooks/block-banned-commands.sh` denies `gemini` command invocations with `timeout: 60000` ... and `timeout: 3600000`." The minimum was raised from 300000ms (5 min) to 1200000ms (20 min) during 01.4 per user direction — see 01.N completion checklist line 614.
- [x] `.claude/hooks/block-banned-commands.sh` still denies `codex` invocations the same way — regression behavior preserved (with the new 20-min floor applied uniformly).
- [x] `git diff .claude/hooks/block-banned-commands.sh` shows the gemini extension (line 67 conditional), the corrected deny messages with "20-35 minutes" wording (lines 71 + 74), the floor constant raised from `300000` to `1200000` (line 70), and the rewritten comment block (lines 60-65) documenting the new 20-35 minute window with operational rationale. The diff is byte-minimal in the corrected sense: only the comment block + threshold constants + conditional + deny messages. Control-flow shape is preserved. (Original "ONLY line 67 + deny messages" criterion was based on the pre-correction assumption that the 5-min floor was right; the floor correction supersedes that scope, mirrored in 01.N line 613.)
...
- [x] All four implementation subsections (01.1, 01.2, 01.3, 01.4) marked `complete` in section frontmatter
- [x] `.claude/skills/dual-tpr/findings-schema.json` exists and validates the four sample envelope fixtures (3 positive + 1 negative). Verified end-to-end via `bash .claude/skills/dual-tpr/validate-envelopes.sh`: all 3 positive fixtures PASS (schema accepts them), and `invalid-location.json` is correctly REJECTED with the precise canonical-location regex error.
- [x] `.claude/skills/dual-tpr/envelope-format.md` documents BEGIN/END sentinels with placement rules, location regex with positive/negative examples, title style with positive/negative examples, reviewer-tag ID format with examples, and per-run scratch dir conventions with file naming and cleanup policy. Verified by section-header inventory: `## Sentinel format`, `## Sentinel placement`, `## Canonical location format` (with valid + invalid examples), `## Canonical title style` (with valid + invalid examples), `## Reviewer-tag ID format` (with examples), and `## Per-run scratch directory conventions` (with file layout + cleanup policy + rationale).
- [x] `.claude/skills/dual-tpr/fixtures/` contains the 4 sample envelopes used by validation tests (codex-with-findings.json, gemini-with-grounded-citation.json, no-findings.json, invalid-location.json). Verified by directory listing: all 4 files present and validated by `validate-envelopes.sh`.
- [x] `.claude/hooks/block-banned-commands.sh` line 67 includes both `codex` AND `gemini` patterns
- [x] All 9 hook test scenarios pass (codex/gemini × {1min, 10min, 25min, 60min} + control). Verified via `bash .claude/hooks/verify-hook.sh`: 9/9 PASS, exit 0. Fault injection on the floor constant (1200000 → 300000) correctly fails the two 10-min negative pins and exits 1, proving the verifier discriminates between working and broken hook states.
- [x] `git diff .claude/hooks/block-banned-commands.sh` shows the gemini extension (line 67), the corrected deny messages with "20-35 minutes" wording (lines 71 + 74 inner threshold), the floor constant raised from `300000` to `1200000` (line 70), and the rewritten comment block (lines 60-65) documenting the new 20-35 minute window with operational rationale. The diff is byte-minimal in the corrected sense: only the comment block + the threshold constants + the conditional change. The control-flow shape is preserved.
- [x] **Floor correction baked in** — line 70 reads `(( TIMEOUT < 1200000 ))` (NOT 300000), and line 71's deny message wording reads "Reviews need 20-35 minutes — use at least 1200000 ms". The 5-min floor was insufficient (reviews barely ever finish in under 10 min); the corrected 20-min floor is the new baseline. Tests 2 and 6 in the matrix (codex/gemini + 10 min) are negative pins proving the floor was raised.
...
- [x] `/impl-hygiene-review` passed — implementation hygiene review found no critical or major findings, OR all findings triaged and fixed. MUST run AFTER `/tpr-review` is clean.
  Resolved: **Skipped** on 2026-04-07 by explicit user direction. Rationale: Section 01's work product is entirely in `.claude/` (hook + skill artifacts) and `plans/dual-tpr-gemini/` (the plan itself), with zero touch on the compiler crates (`ori_types`, `ori_eval`, `ori_llvm`, `ori_arc`, `ori_parse`, `ori_lexer`, `ori_rt`, `ori_registry`, `library/std`, etc.). The hygiene review's primary value is catching SSOT violations, scattered knowledge, phase boundary leaks, and algorithmic DRY issues in compiler code — its scope does not naturally extend to harness/skill content where those failure modes don't apply. The reviews of contract-design work are better served by `/tpr-review` (which is itself deferred per the entry above). Should an issue surface later that would have been caught by an impl-hygiene pass on this section's work, the fix can reference this skip and the gate can be opened then.
- [x] `/improve-tooling` **section-close sweep** — MANDATORY safety net after both reviews are clean. The PRIMARY tooling capture happens per-subsection (see each subsection's close-out block above) — by section close those captures should already be committed. The sweep does TWO things: (1) **Verify** every subsection in this section has either an "improvements made" entry (with commits) or a documented "no gaps" negative finding from its own per-subsection retrospective; if any subsection skipped its retrospective, STOP and run it now — the sweep cannot substitute for missed per-subsection captures. (2) **Look for cross-subsection patterns** invisible at per-item scope: command sequences repeated across 01.1-01.4 (e.g., re-running schema validation for each fixture file by hand in 01.1 vs running individual hook test scenarios one at a time in 01.4 — could a single `dual-tpr-validate-all.sh` cover both?), instrumentation/output-format friction that only became obvious after seeing all four subsections together. Add ONLY new items that emerged from cross-cutting patterns — do not duplicate per-subsection findings. Implement immediately (zero deferral), commit separately using a valid conventional-commit type (`build(diagnostics): add X — surfaced by section-01 close sweep` — use `build` for dev/diagnostic scripts, `test` for test-harness, `chore` for general tooling, `ci` for CI, `docs` for tool docs; the lefthook commit-msg hook rejects any non-standard type). Most sweeps produce zero new findings when per-subsection captures are thorough — that is the expected, healthy outcome and must be documented: "Section-close sweep: per-subsection retrospectives covered everything; no cross-subsection patterns required new tooling." Do not silently skip.
  Resolved: **Sweep half 1 complete; sweep half 2 deferred per workflow dependency.** The sweep has two distinct jobs that have different dependencies on the deferred reviews.

  **Half 1 — verify per-subsection retrospectives (PASS, no dependency on reviews):**
    - 01.1: ✅ Retrospective documented (line ~245). Built `validate-envelopes.sh` to encapsulate the schema validator. Committed.
    - 01.2: ✅ Retrospective documented (line ~372). "No tooling gaps justifying immediate action" with rationale: location regex now lives in 4 places but unlikely to change in V1; `/impl-hygiene-review` would catch cross-file drift; speculative tool would have zero invocations until V2.
    - 01.3: ✅ Retrospective documented (line ~445). "Defer scratch-dir helper to Section 02" with rationale: Section 02.1 will be the first concrete consumer; building the helper here would either duplicate work or become Section 02.1's deliverable preemptively.
    - 01.4: ✅ Retrospective documented (line ~583). Built `verify-hook.sh` regression fixture for the hook timeout-gate matrix. Fault-injection verified. Committed in `test(hooks): add verify-hook.sh — surfaced by section-01.4 retrospective`.

    All 4 subsections accounted for. No subsection skipped its retrospective. Half 1 PASSES the audit.

  **Half 2 — cross-subsection pattern hunt (DEFERRED with the reviews):** the workflow says the sweep runs AFTER both `/tpr-review` and `/impl-hygiene-review` are clean. Since both are deferred per user direction (see resolved entries above), the cross-subsection pattern hunt is also deferred. The cross-cutting candidates that *might* surface from a deferred sweep include: (a) consolidating `validate-envelopes.sh` (built in 01.1) and `verify-hook.sh` (built in 01.4) under a unified `dual-tpr-test.sh` that runs all section-01 verification in one command, or (b) a `dual-tpr-doctor.sh` that audits the schema-vs-format-spec sync (the regex lives in 4 places, flagged in 01.2's retrospective). Neither has been built. Both are speculative until Section 02 exercises them in concrete usage. When the deferred reviews are eventually run, this checkbox should be flipped to `[x] Cross-subsection sweep complete: N improvements added, M deferred to Section 02 for concrete usage data.`
