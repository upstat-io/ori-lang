...
success_criteria:
  ...
  - "Negative disk-vs-graph check: every `plans/<dir>/` with `index.md` (excluding `completed/`) produces a corresponding `:Plan{id: <dir>}` node in the graph. Verified by `sync-plan-bug-graph.sh --full` logging `INFO: disk-vs-graph verification: OK: N disk plans == N graph plans`, and by `sync-plan-bug-graph.sh --full --verify-strict` exiting 0 (exit 1 signals drift). Amended 2026-04-18 after `plans/plan-bug-dag-ingestion-gap-fix/` — the original §06 had 10 positive exit criteria but no negative check, and three plans (`project-reorganization`, `sync-docs-redesign`, `roadmap`) were silently dropped because their `index.md` files lacked YAML frontmatter, or their section files tripped a spurious `[&*]\w` regex on `**bold**` markdown."
inspired_by:
...
- [x] **Negative disk-vs-graph check** (added 2026-04-18 by `plans/plan-bug-dag-ingestion-gap-fix/`): every `plans/<dir>/` with `index.md` (excluding `completed/`) corresponds to a `:Plan{id: <dir>}` node. Enforced by the `sync-plan-bug-graph.sh` post-sync verification step (WARN on drift, exit 1 under `--verify-strict`) and by the `plan_corpus export` admission gate (exit 1 if any `plans/*/index.md` or `plans/*/00-overview.md` has a parse_error). Live run 2026-04-18: `25 disk plans == 25 graph plans` after adding frontmatter to `project-reorganization/index.md`, `sync-docs-redesign/index.md`, and `roadmap/index.md`.

**Amendment — 2026-04-18.** The original §06 checklist shipped with 10 positive exit criteria but no *negative* check of the form "every disk plan corresponds to a graph node." As a result, three plans (`project-reorganization`, `sync-docs-redesign`, `roadmap`) were silently dropped from the graph throughout the original plan's lifetime — either because their `index.md` files had no YAML frontmatter at all, or because section files tripped a spurious `[&*]\w` regex in `scripts/plan_corpus/types.py:28` on `**bold**` markdown and `**/build` glob text inside quoted string values. The regex has been replaced with a `_NoDuplicateKeyLoader.compose_node` hook in `scripts/plan_corpus/parser.py` that rejects real YAML anchors/aliases at the composer layer (where PyYAML can tell them apart from string-literal text). The `plan_corpus export` subcommand now refuses to emit (exit 1) when any `plans/*/index.md` or `plans/*/00-overview.md` has a `parse_error` finding, preventing partial graph imports. See `plans/plan-bug-dag-ingestion-gap-fix/` for the full fix plan and `/home/eric/.claude/plans/sparkling-sniffing-quiche.md` for the executed Claude Code plan document.
