...
- **NOTE (drift with §05.2 — case (c)):** `plans/test-suite-health/00-overview.md:5` has `supersedes: []` and `plans/test-suite-health/index.md` is NOT a reroute. The live plan is "reprioritize and update 21A in place," not "supersede 21A." §02.2 SUPERSEDED must cover BOTH (i) reroute-plans-with-supersedes-list AND (ii) in-place-rewrite-claimed-but-not-done patterns — otherwise §05.2's case (c) is plan-unimplementable. Downstream /tpr-review to reconcile with §05.2:124 if the acceptance bar changes.
...
- [ ] Collect body-text, HTML-comment, and YAML-comment references (NO edges created):
  - For each node's body (`ValidatedFile.body`):
    ...
    - Define `DEPENDENCY_VERBS = frozenset({"depends on", "requires", "blocked by", "prerequisite", "unblocks", "supersedes", "rewrites", "obsoletes"})` (verbs that imply a dependency or supersession edge).
...
- [ ] **SUPERSEDED** — reroute claims OR in-place-rewrite claims with incomplete rewrites:
  ...
  - Case (ii) — in-place-rewrite claim (Finding A): for each plan section whose body prose contains `rewrites <target>` or `rewrite of <target>` (matching a `DEPENDENCY_VERBS` rewrite reference), verify that the target section has been edited since the claim. If the target's frontmatter `reviewed: false` AND no body changes reference the rewrite claim → emit finding.
  - Known case (§05.2 (c)): `plans/test-suite-health/section-02-roadmap-reprioritization.md` claims to rewrite `plans/roadmap/section-21A-llvm.md` but the rewrite has not landed. `plans/test-suite-health/00-overview.md:5` has `supersedes: []` and `index.md` is NOT a reroute — so case (ii) is the correct detection path.
...
- [ ] **BLOCKED** — active node depends on queued/not-started node:
  ...
  - Known cases (§05.2 (a), (g)): repr-opt active, locality prerequisite queued; BUG-04-039 in-progress, blocked by `plans/iterator-element-ownership/` (recorded via YAML frontmatter comment per §02.0).
  - **Status resolution helper** — because §01.4 owns the canonical status normalizer, §02.2 imports `plan_corpus.normalizer.effective_status(node)` and uses the normalized status, not the raw frontmatter value. If §01.4 does NOT yet expose this, §02 calls the normalizer directly on the node's frontmatter and records a NOTE in §02.N for /tpr-review.
...
- [ ] **DEAD_REFERENCE** — pointer to nonexistent plan/directory/spec:
  ...
  - Known cases (§05.2 (b), (h)):
    - (b) `plans/roadmap/section-22-frontend-core.md:141` references `plans/ori_lsp/` (must verify exact line at runtime; `plans/ori_lsp/` does not exist on disk)
...
- [ ] Implement transitive priority inversion detection:
  ...
  - Emit one Finding per chain with `evidence = tuple(f"{node.path}:{node.status}" for node in chain)` — the structured chain-as-evidence encoding per §02.5 Option C (or the schema-extended encoding if §01.3 adopts Option A).
...
- [ ] **CUT (Finding L): topological-sort execution-order recommendation is NOT emitted by §02.**
  ...
  - Keep `dag.edges` + the DAG's transitive closure available — if a future §03 subsection adopts topo-sort rendering, it can compute on demand.
  - The topo-sort code is CUT from §02, not deferred — if needed later, it lives in the consumer, not here.
...
- [ ] Validate against known test cases (concrete assertions for §05.2):
  ...
  - Test case (b): DEAD_REFERENCE for `plans/ori_lsp/` in `plans/roadmap/section-22-frontend-core.md`.
...
- [ ] **Concern I — multi-hop transitive chains (Finding I):**
  ...
  - **Options (pick one, document choice):**
    - (A) Extend §01.3 `Finding` with optional `dependency_chain: tuple[Path, ...] = ()` — requires a §01.3 extension (NOTE for /tpr-review).
    - (B) Emit multiple Findings per edge in the chain, stitched in §03 via a shared chain-id stored in `evidence[0]`.
    - (C) Encode chain as structured path tuples inside `evidence`, e.g. `evidence=("chain:A,B,C", "status:active,queued,not-started")`.
  - **§02 choice: Option C** as the default. Rationale: avoids extending §01.3's Finding schema (§01 is complete), keeps §02 self-contained, and §03's report renderer can split on `"chain:"` prefix. If /tpr-review ratifies Option A during §02 implementation, swap to Option A via the §01.3-edit rule documented at §02.2's "Classifier subtypes not declared in §01.3" bullet.
...
- [ ] §02.2 `FindingSubtype.REDUNDANT_DEPENDENCY` and `FindingSubtype.ORPHANED_PLAN` added to §01.3's `types.py` (single cross-section edit permitted during §02 execution)
- [ ] §02.3 Transitive chains via Option C evidence-encoding; topological sort CUT (Finding L) — no consumer
- [ ] §02.4 Classifier precedence documented and TDD-enforced; deterministic ordering; code-fence negative pin passes
- [ ] §02.5 Handoff contract with §03 resolved: Option C for chains, source_column disambiguator for Finding.id, enriched resolve_dep findings, source_kind embedded in evidence
