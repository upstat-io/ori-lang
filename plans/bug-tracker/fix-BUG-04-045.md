...
- [x] **Step 15 — Fix missed test assertion in `test_cross_compile_linux_to_macos`** (completion of Step 12's "Update all consumer tests" directive, surfaced 2026-04-08 by the lefthook pre-commit hook during dual-tpr-gemini §05.2 Scenario 2 work).
  **Root cause**: Step 12 updated `test_x86_64_linux_data_layout` (cross.rs:500), `test_aarch64_macos_data_layout` (cross.rs:569), and the per-arch predicate tests to use `assert_eq!(config.pointer_size(), 8)` instead of `layout.contains("p:64:64")`, because modern LLVM omits the default-address-space `p:64:64` from x86_64-apple-darwin and x86_64-linux data layout strings. Both updated tests added an explicit comment explaining the rationale. **But `test_cross_compile_linux_to_macos` (cross.rs:614) was missed** — it still used the stale `layout.contains("p:64:64")` pattern against a `macos_target()` config (triple `x86_64-apple-darwin`), and therefore broke as soon as the test suite ran against modern LLVM's x86_64-apple-darwin layout.
  **Why it wasn't caught during the main refactor**: `test_cross_compile_linux_to_macos` lives in a different region of the file from the data-layout-focused tests and exercises cross-compile semantics rather than data-layout assertions primarily — the layout check is a secondary assertion guarded by `if let Ok(layout) = ... { ... }`. Step 12's grep over the test file for `layout.contains("p:64:64")` must have missed this cell because it was buried inside a conditional block whose outer context was "cross-compile", not "data layout".
  **Fix**: Replaced the stale `layout.contains("p:64:64")` assertion with `assert_eq!(target.pointer_size(), 8)` following the pattern already established in `test_x86_64_linux_data_layout:517` and `test_aarch64_macos_data_layout:583`. Added a comment referencing both sibling tests and noting the connection to BUG-04-045's Arch-enum refactor, so a future reader can trace the evolution of this pattern. The fix is structurally a one-line assertion swap wrapped in a five-line explanatory comment.
  **Verification**: `timeout 150 cargo test -p ori_llvm --test aot test_cross_compile_linux_to_macos` → PASS (was FAIL before the fix). Full `./test-all.sh` via the lefthook pre-commit hook will run on commit.
  **TDD matrix completeness**: The matrix ALREADY exists in the 3 sibling tests (`test_x86_64_linux_data_layout`, `test_aarch64_macos_data_layout`, `test_wasm32_data_layout`) plus the dedicated `test_pointer_sizes` test at cross.rs:768 that enumerates every target. This fix completes the missing cell. No new matrix tests were added because adding a 5th assertion in the same file for the same pattern would be algorithmic duplication — the principle is already enforced by the existing matrix, and the `test_pointer_sizes` enumeration test at cross.rs:768-777 is the dedicated pointer-size matrix that would fail if the fix pattern is ever reverted across ALL targets simultaneously.
