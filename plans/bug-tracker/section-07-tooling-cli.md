...
- [x] `[BUG-07-015][high]` **LLVM test-backend stack-overflows when ARC lowering emits errors with live test cases** — found by /roadmap-work §02.3 close-out pre-commit hook; fixed 2026-04-17.
  Resolved: Fixed in this commit. `compiler/oric/src/test/runner/arc_lowering.rs::lower_and_infer_borrows` now returns `Option<(annotated_sigs, arc_cache)>` and `None` on ARC errors; `llvm_backend.rs::run_file_llvm` handles `None` by raising `LLVMEvalError` instead of calling `compile_module_with_tests` with empty maps. Reproducible via `./target/release/ori test --backend=llvm tests/run-pass/rosetta/001_100_doors/` before the fix; clean `LlvmCompileFail` outcomes after.
  Repro (pre-fix): `./target/release/ori test --backend=llvm tests/run-pass/rosetta/001_100_doors/` → E4003 at span 533..557 ("index assignment reached ARC lowering before desugaring") followed by `thread 'ori-main' has overflowed its stack, fatal runtime error`. Root cause: `lower_and_infer_borrows` returned `(empty_sigs, empty_cache)` on ARC errors but the caller couldn't distinguish "empty due to error" from "empty because no functions"; compile_module_with_tests then recursed trying to resolve callees referenced by tests but missing from the empty cache.
  Follow-up (BUG-07-016 below): implement `typeck.md §EX-17` index-assignment desugar (`list[i] = x` → `list = list.updated(key: i, value: x)`) so `001_100_doors.ori` actually compiles via LLVM instead of emitting `LlvmCompileFail`. That is a language-feature gap (`canon.md §2` flags EX-17 as target-only); BUG-07-015 is the crash-prevention fix only.
  Subsystem: `compiler/oric/src/test/runner/` (error-path contract between lowering + compile)
  Found: 2026-04-17 | Source: /roadmap-work §02.3 pre-commit hook triggered ./test-all.sh and surfaced the crash.

- [ ] `[BUG-07-016][medium]` **`typeck.md §EX-17` index-assignment desugar not implemented — `list[i] = x` raises E4003 instead of rewriting to `list = list.updated(key: i, value: x)`** — found alongside BUG-07-015.
  Repro: any program with `some_list[i] = value` reaches ARC lowering and emits E4003; the evaluator also raises `index_assignment_not_supported`. Spec (`docs/ori_lang/v2026/spec/` Clause 8 Collections / `typeck.md §EX-17`) specifies the desugar at the type checker; `canon.md §2` flags the rewrite as **target-only**. `tests/run-pass/rosetta/001_100_doors/001_100_doors.ori` is the canonical affected test.
  Subsystem: `compiler/ori_types/src/infer/expr/` (where EX-17 rewrite belongs), `compiler/ori_canon/src/lower/expr.rs` (currently lowers `Assign { Index }` verbatim — should not reach canonicalization once EX-17 ships), `compiler/ori_eval/src/interpreter/can_eval/control_flow.rs::index_assignment_not_supported` (eval-side mirror), `compiler/ori_arc/src/lower/control_flow/mod.rs::lower_assign` (the E4003 emit site — would become dead code).
  Found: 2026-04-17 | Source: /roadmap-work §02.3 close-out surfaced the gap via BUG-07-015's crash.
  Note: needs full /fix-bug rigor when picked up — touches typeck + canon + eval + LLVM + spec conformance tests. Field-assignment (`state.f = x` → `state = { ...state, f: x }`) has the same target-only status and should be implemented in the same plan.
