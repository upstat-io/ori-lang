...
- [ ] `[BUG-04-030][high]` **LLVM JIT spec tests: LCFails from multiple pre-existing codegen issues** — found by continue-roadmap.
  ...
  Note: This is a meta-bug. Root cause (A) is the dominant contributor (~90% of LCFails). The fix requires deep work in the imported mono function pipeline (`body_type_map` construction in `llvm_backend.rs`, type var resolution chain in `pool/accessors.rs`). Root causes (B) and (C) are cascading symptoms of (A). Root cause (D) is OBE. Root cause (E) needs type structure matching beyond arity. Root cause (F) is AOT-specific.
