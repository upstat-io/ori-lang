...
- [ ] `[BUG-04-089][high]` **LLVM backend: 3 spec-tests regressed to runtime failures after §04.2.B unblocked LC_FAIL tests**
  Repro: `timeout 150 ./test-all.sh` at HEAD `8a7e9040` → `Ori spec (LLVM backend)` = `2392 passed / 4 failed / 27 skipped / 2078 lc_fail`. Prior state.sh baseline at `58c26963` was `1851 passed / 1 failed / 21 skipped / 2615 lc_fail`. The root-cause fix `3f7d85f5 fix(typeck): §04.2.B root cause — extend var_subst with union-find roots` unblocked ~541 previously-LC_FAIL tests (passed 1851 → 2392, lc_fail 2615 → 2078); 3 of those newly-reaching-runtime tests hit latent LLVM codegen/runtime bugs that were masked while the typeck var-substitution issue short-circuited them at LC_FAIL.
  Failures (all under `--backend=llvm`):
  - `tests/spec/expressions/immutable_bindings.ori::test_list_immutable` — assertion failed: `6 != 3`. `let items = [1, 2, 3]; let [$first, $second, ..rest] = items; first + second` returns 6 instead of 3. Duplicate coverage of the same LLVM pattern-destructuring bug tracked as `BUG-04-086`; this entry captures the test's reappearance at the runtime tier after §04.2.B unblocked it.
  - `tests/spec/patterns/catch.ori::test_catch_div_zero` — division by zero. `catch(expr: 1 / 0)` lets the panic escape instead of producing `Err`. Duplicate coverage of `BUG-04-087`.
  - `tests/spec/traits/debug/collections.ori::test_map_debug` — assertion failed: `"{x: 1}" != "{\"x\": 1}"`. Map debug formatter omits string-escape on keys. Duplicate coverage of `BUG-04-088`.
  Subsystem: `compiler/ori_llvm/` (pattern-destructuring codegen, catch-pattern landing-pad codegen, map-debug keyed-formatter codegen)
  Notes:
  - NOT a §04.2.B regression — `3f7d85f5` landed a clean typeck fix; these are pre-existing latent LLVM bugs that were masked while ~541 tests were hitting LC_FAIL upstream.
  - The 4th failure in the `4 failed` summary tally is the 1 pre-existing LLVM failure from the prior baseline (`58c26963`) — retained as-is, not captured in this bug.
  - This bug is an aggregate anchor — the three failing tests already have individual open bugs (`BUG-04-086`, `BUG-04-087`, `BUG-04-088`). This entry exists to document the §04.2.B unblocking event and capture the delta vs. the state.sh baseline. Individual fixes close via the existing per-test bugs; this entry closes when all three are resolved (or when the aggregate is absorbed into their closure notes).
...
- [ ] `[BUG-04-090][high]` **AOT codegen: generic forwarder applied to `[T]` causes `ori_rc_dec` on already-freed allocation**
  Repro (minimal, 1-hop): `@id<T> (x: T) -> T = x; let xs = id(x: [1,2,3]); xs.len()` — aborts with `ori: FATAL — ori_rc_dec called on already-freed allocation at …` (exit 134 / SIGSEGV).
  Reproduces at 2-hop as well — existing `generic_calling_generic` shape applied to `[int]` — so this is NOT a §04.2.B regression. The root-extension fix `3f7d85f5` (var_subst union-find roots) landed cleanly; the `[T]`-through-generic RC path was already broken before §04.2.B and only became reachable in tests after §04.2.B unblocked codegen.
  Fixture + ignored AOT test: `compiler/ori_llvm/tests/aot/fixtures/generics/generic_chain_list_element.ori` + `compiler/ori_llvm/tests/aot/generics.rs::test_generic_chain_list_element` (currently `#[ignore]`; will green when this bug lands).
  Subsystem: `compiler/ori_llvm/src/codegen/arc_emitter/` — RC emission for list return values from monomorphized generic forwarders. Likely root-cause area: the mono-instance's return-RC decision for `[T]` passes the list allocation ownership twice (caller path + forwarder path), yielding a double-decrement at the caller's scope exit.
  Found: 2026-04-21 | Source: manual (close-out of `plans/empty-container-typeck-phase-contract/section-04-codegen-assertions.md §04.2.B`; minimal-repro reduced during §04.2.B close-out from a fixture family of 10 new generic-chain AOT test cases)

- [ ] `[BUG-04-091][high]` **AOT codegen: inherent method `@unwrap (self) -> T` on generic type `Box<T>` fails with `unresolved function 'unwrap' in apply — missing mono instance?`**
  Repro: `impl<T> Box<T> { @unwrap (self) -> T = self.inner }` — no method-level generics, just an inherent method on a generic type. Typeck accepts; codegen fails with `unresolved function 'unwrap' in apply — missing mono instance?` followed by `E5001 LLVM module verification failed`. This is the reduced shape of `BUG-01-002`'s blocker — with method-level generics stripped, the parser accepts the input, but codegen still can't resolve the inherent method on the generic type through the mono-instance pipeline.
  Subsystem: `compiler/ori_llvm/src/codegen/arc_emitter/apply.rs` — `apply` cannot resolve inherent methods on generic types through the mono-instance pipeline. Likely root-cause area: `collect_mono_functions` / mono-instance registration for `impl<T> Type<T> { @method }` inherent-method shape; the call-site-to-mono-instance mapping is missing the inherent-method-on-generic-type case (related to `BUG-04-AOT-MONO`'s imported-generic gap but distinct — this is a local-module inherent-method gap).
  Test case: same ignored test as `BUG-01-002` — `compiler/ori_llvm/tests/aot/generics.rs::test_generic_method_on_generic_type` (currently `#[ignore]`). The test blocks on BOTH `BUG-01-002` (parser) and this bug (codegen).
  Found: 2026-04-21 | Source: manual (close-out of `plans/empty-container-typeck-phase-contract/section-04-codegen-assertions.md §04.2.B`)
...
- [ ] `[BUG-04-AOT-MONO][high]` **AOT `ori build` lacks imported generic monomorphization — `collect_mono_functions` does not traverse `import_sigs` for mono-instance emission**
  ...
  Evidence (direct inspection, 2026-04-19): `ori build` goes through `compiler/oric/src/commands/compile_common.rs:70-110,184-240` + `compiler/ori_types/src/check/mod.rs:341-407` with NO `re_intern_*` call. `compiler/ori_llvm/src/monomorphize/mod.rs::collect_mono_functions` operates only on the host-module sig map and does not traverse imported-module sigs for mono-instance emission. Likely fix surface: extend `collect_mono_functions` to walk `import_sigs` and emit one mono instance per `(generic_sig, instance_args)` pair reachable from host-module callers, then ensure the driver registers those mono-instance LLVM names into the link surface so the imported-generic call site at the host module resolves.
  Subsystem: `compiler/ori_llvm/src/monomorphize/`, `compiler/oric/src/commands/build/multi.rs`, `compiler/oric/src/commands/compile_common.rs`.
  Related: sibling of BUG-04-042 (absorbed into `plans/empty-container-typeck-phase-contract/section-08-codegen-poly-lambda.md` §08 — JIT-only scope). AOT integration tests at `compiler/ori_llvm/tests/aot/poly_lambda_mono.rs` are retained in-tree as failing TDD pins for THIS bug; they will green when this bug lands, not as part of §08.
