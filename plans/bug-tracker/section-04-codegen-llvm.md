...
- [ ] `[BUG-04-050][low]` **`compiler/ori_arc/src/aims/realize/emit_unified.rs` is 540 lines (over 500-line limit per impl-hygiene §File Organization)** — found by impl-hygiene-review (BUG-04-047 post-TPR-clean pass).
  Repro: `wc -l compiler/ori_arc/src/aims/realize/emit_unified.rs` reports 540. The file has been over 500 lines since before the BUG-04-047 fix landed — that fix actually SHRUNK it by 2 lines (removed the `uses_so_far,` destructure at line 242 and the same field from the `emit_terminator_rc` call site). Per `impl-hygiene.md §File Organization`: "500-line limit: source files (excluding tests); exceeding = BLOAT finding. Split when touching: touching a file over 500 lines without splitting = finding." Technically the BUG-04-047 fix touched the file, so this is a finding; but the touch was a net-negative edit (shrinking the file), and splitting a 540-line file with multiple orchestration responsibilities is out of scope for the BUG-04-047 fix.
  Impact: low. No correctness issue, no duplication, no side logic — just a file size limit violation on a pre-existing orchestration file. The file contains the unified AIMS realization pipeline's per-function orchestration (`realize_function_emit` and helpers), which is inherently coordinating multiple phases. A clean split would probably extract one of: the `BlockCtx` assembly (currently inline in `realize_function_emit`), the `route_merge_edge_decs` helper cluster, or the `collect_dead_invoke_dsts` / `collect_invoke_edge_decs` pair (though these live in `edge_cleanup.rs` today — checking their orchestration home).
  Suggested fix: `realize_function_emit` is the largest function in the file (~80 lines). Extract the pre-walk setup (BlockCtx assembly, precomputation of use_info, defined_in_block, borrowed_defs, child_elu) into a `prepare_block_ctx` helper in `forward_walk/mod.rs` or a new `ctx_builder.rs` submodule. That should shrink the file by ~30-50 lines, enough to get under 500.
  Subsystem: `compiler/ori_arc/src/aims/realize/emit_unified.rs`
  Found: 2026-04-09 | Source: impl-hygiene-review (BUG-04-047 hygiene pass)
  Note: Pre-existing over-limit; touched by BUG-04-047 fix but not grown. Filed here so the plan-annotations scanner has an active `[ ]` entry for the anchor.

- [ ] `[BUG-04-048][medium]` **`forward_walk.rs` hand-rolled Invoke/InvokeIndirect owned-position check disagrees with canonical `ArcTerminator::is_owned_position`** — found by continue-roadmap (surfaced during BUG-04-047 /tp-help consensus).
...
- [ ] `[BUG-04-049][low]` **`ArcInstr::ApplyIndirect` and `ArcTerminator::InvokeIndirect` use opposite `used_vars()` argument orderings (`[closure, ...args]` vs `[...args, closure]`)** — found by continue-roadmap (surfaced during BUG-04-047 /tp-help consensus).
