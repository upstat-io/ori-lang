...
- [x] **Subsection close-out (05.2)** — MANDATORY before section completion:
  ...
  - [x] Run `/improve-tooling` retrospectively — see the comprehensive retrospective below.
    ...
    **Carry-forward for Section 07:**

    - Section 07 (/tp-help dual-source + consolidation) will inherit the full lint coverage + schema-validation infrastructure. The new schema-validation phase catches a class of errors (template semantic drift) that the phrase-presence lint cannot.

    - Section 07's rewrite should START from the corrected review-work template — all the round-1/2/3 findings are already fixed in the current state. The "Minimal envelope template" pattern with scoped schema validation is the recommended template structure for any future SKILL.md that embeds JSON examples.
    ...
    - Section 06 (originally "/review-plan new Claude skill") was removed 2026-04-08 as redundant — once Section 07 makes `/tp-help` dual-source, plan review reaches dual-source by asking `/tp-help` to review a plan directory. The separate wrapper became architectural duplication. The `.codex/skills/review-plan/SKILL.md` and `.gemini/skills/review-plan/SKILL.md` reviewer-side skills (created in §03) remain for standalone `codex exec /review-plan` and for `/tp-help` dispatch.
...
- [x] `/improve-tooling` section-close sweep done
  ...
  **Half 2 — cross-subsection pattern hunt (PASS, concrete improvements):**
    ...
    - **Cross-wrapper DRY awareness**: both rounds 1 and 2 surfaced findings that applied to BOTH tpr-review/SKILL.md AND review-work/SKILL.md. The plan's Step 1E command-file boundary decision accepts the duplication between the two wrappers, but it means fixes must always cover both. Future Section 07 (tp-help consolidation) should follow the same dual-fix pattern. _(Section 06 originally carried the same warning for the `/review-plan` skill rewrite, but Section 06 was removed 2026-04-08 as redundant — see 05.2 retrospective carry-forward for the full removal rationale.)_
