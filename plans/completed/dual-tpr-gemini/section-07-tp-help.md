...
- [x] Read `.claude/skills/impl-hygiene-review/SKILL.md` lines 319-360 to understand exactly how it calls `/tp-help` and processes the response. **Findings (2026-04-08):** Phase 4 has two invocations: **4a Validate Findings** (line 327) passes 5-10 findings and asks the reviewer to validate; the wrapper's prose-parsing logic is "If Codex confirms... challenges... surfaces NEW findings..." — pure prose, no structural parsing. **4b Probe Blind Spots** (line 344) asks the reviewer to look at specific under-examined areas. **4c Integrate** (line 362) tags confirmed findings with `[TP-CONFIRMED]` and newly-surfaced findings with `[TP-SURFACED]`. **Dual-source compatibility: SAFE** — the consumer reads `/tp-help` output as free-form prose. Under dual-source concat, the wrapper simply sees BOTH reviewers' validation/challenge/surface statements adjacent. If EITHER reviewer confirms a finding → `[TP-CONFIRMED]`; if EITHER surfaces new → `[TP-SURFACED]`. The existing tagging semantics are preserved because the tags ask "did any external reviewer confirm/surface?" — the answer is yes/no regardless of reviewer count. Leakage risk: only if the wrapper writes raw `/tp-help` text verbatim into findings, the HTML-comment sentinels would leak — negative pin asserted in Scenario 1.

- [x] Read `.claude/commands/review-plan.md` lines 95-135 (Step 3B) and lines 305-340 (Midpoint Check) to understand how the command file builds its `/tp-help` prompts and consumes the responses. **Findings (2026-04-08):** **Step 3B** (lines 95-112) is a SEQUENTIAL + FOREGROUND blind-spot check before the 4 review agents launch; its usage pattern is "Use Codex's response to inform the review — add specific items to watch for in the agent prompts if Codex identifies something non-obvious that the standard review lenses might miss." The consumer extracts "items to watch for" as prose and injects them into subsequent Agent prompts. **Midpoint Check** (lines 305-321) runs between Agent 2 and Agent 3; same pattern — "Feed relevant insights into Agent 3's prompt as additional focus areas." Both calls are explicitly SEQUENTIAL + FOREGROUND (lines 97 + 307). **Dual-source compatibility: SAFE** — the 4-agent pipeline never parses `/tp-help` output for structure; it extracts "items to watch" or "insights" as free-form prose and injects them into the next Agent's system prompt. Under dual-source, the Agent prompts will see richer context (two reviewers' insights instead of one). The Agent prompts themselves don't break on seeing "Codex says X. Gemini says Y." Leakage risk: the 4-agent pipeline writes findings to plan section TPR blocks — if any Agent pastes raw `/tp-help` output into a plan file, HTML-comment sentinels would leak. Negative pin asserted in Scenario 2.

- [x] Read `.claude/skills/create-plan/SKILL.md` lines 56, 130-170 (Phase 1 research loop), 520-540 (Phase 3 architectural sanity check), and 578-600 (Step 8B architecture sanity check) to understand how create-plan builds and consumes `/tp-help` responses at each call site. Note the common pattern: create-plan calls `/tp-help` in the foreground, waits for the full response, then includes the response text (or a summarized extract of it) as context for the next research pass or the next workflow phase. Because §07.2 uses HTML-comment sentinel attribution (NOT H2 headers), the concerns around heading-level collision are avoided BY CONSTRUCTION — the sentinels cannot appear as Markdown structure. Scenario 4 still verifies end-to-end: it must confirm (a) create-plan's prose rendering correctly integrates the raw reviewer text, (b) no HTML-comment sentinels leak into the final written plan document, and (c) no doubled-text or truncation artifacts appear. **Findings (2026-04-08):** **Line 56** is a general sequencing rule (no invocation — declarative only; §08.3 owns the wording update). **Step 1D Consensus Loop** (lines 130-170) is the most interesting site: it is a LOOP with Round 1 and Round 2+ until "You and Codex agree" (line 172). Round 1 prompt (line 143) presents mission + blockers + direction; Round 2+ prompt (line 166) presents agreed + iterating + updated mission. Loop termination: "you and Codex agree on mission direction, scope, approach, integration points" AND "no unresolved disagreements." **Dual-source semantic ambiguity (documented, not a break):** under dual-source the loop becomes "you, Codex, AND Gemini iterate until consensus." If the two reviewers disagree WITH EACH OTHER, the loop termination condition is ambiguous — but the human-author (Claude) still decides when consensus is reached, and the ambiguity surfaces as richer data in the next round's prompt, not a crash. This is a documentation drift (the prose says "Codex" singular), tracked for §08.3's line-56 wording update which should propagate to 1D's wording. **Step 6B** (line 524) is a single foreground call — "second opinion on the architectural direction" — pure prose consumption, more data is always better. **Step 8B** (line 584) is a single foreground call — "sanity-check the written overview architecture" — same pattern. **Dual-source compatibility: SAFE-with-caveat** — no crash risk at any site, but Step 1D's prose needs a future wording update so the loop termination logic explicitly says "both external reviewers" instead of "Codex." Leakage risk is HIGH here because create-plan writes a full `00-overview.md` + section files; if it pastes raw `/tp-help` prose verbatim, sentinels leak. Load-bearing negative pin for Scenario 4 (deferred to a dedicated session per user direction 2026-04-08).
...
- [x] Record all four scenario results in working notes. The working notes MUST include: which scenarios passed (stub AND real), which failed, any attribution-sentinel leakage observed, the wall time for each real scenario (to calibrate user expectations for dual-source `/tp-help` consumers), and confirmation that the Scenario 4 real run used `ORI_TPR_REVIEWERS=both` (NOT `=codex` as a shortcut — the stub harness is where fast iteration happens).
  Results (2026-04-08):
  - **Scenario 1 (`/impl-hygiene-review` on `compiler/oric/src/tracing_setup.rs`)**: PASS. Wall time ~10 min (codex 297s, gemini 600s). All negative pins passed (consumer byte-identical, no sentinel leakage). Dual-source produced substantive prose from BOTH reviewers (10,765 bytes combined), consumer tagging logic worked end-to-end. 5 new compiler findings surfaced, filed as BUG-07-006 through BUG-07-009 (see commit `b4d2905f`).
  - **Scenario 2 (`/review-plan` on copy of `plans/completed/macos-aot-fixes/`)**: PASS-WITH-SCOPE-NOTE. Step 3B /tp-help call ran dual-source end-to-end from inside `/review-plan`'s nested skill context (~7.5 min wall: codex 312s, gemini 17s). Both pipeline fixes verified LIVE during the nested call — polling protocol (commit `71eb89fd`) produced heartbeat output every ~90s with absolute wall-clock anchors, worktree-guard.sh (commit `a36b13c3`) correctly reported CLEAN via new `comm -13` semantics. All 7 negative pins passed (review-plan.md byte-identical with baseline hash match, zero sentinel leakage in TPDIR, no repo drift, no modifications to impl-hygiene-review/create-plan). **Scope note**: User authorized (2026-04-08) declaring Scenario 2 PASS based on the Step 3B dual-source call + comprehensive negative-pin verification rather than running the full 4-agent pipeline + Midpoint /tp-help + verdict. The 4-agent subagent execution was not exercised; the load-bearing assertion "Agent tool subagents parse dual-source concat output without breaking" is inferred from (a) Agent tool subagents consume /tp-help output as free-form prose context, not structured data, (b) HTML-comment sentinels are invisible to markdown renderers and any subagent will treat them as inert, (c) Scenario 1 already proved the prose-compatibility assertion via real impl-hygiene-review Phase 4 consumer tagging. TPDIR cleaned up successfully.
  - **Scenario 3 (byte-identity check on `.claude/commands/review-plan.md`)**: PASS. `git diff --exit-code` returns 0, hash matches frozen §07.PRE baseline `66250250e8030a5e880ceaf4bf40f9409178a375`. Verified during both Scenario 1 and Scenario 2 close-outs.
  - **Scenario 4 (`/create-plan` full run with all 4 internal /tp-help call sites)**: DEFERRED (user authorized at §07.3 start, 2026-04-08) due to ~80-120 min wall-time budget. Mode B (deterministic throwaway slug) is the execution mode per BUG-08-010 still being open. Scheduled for a dedicated session. This scenario is the primary remaining blocker for §07.3 full close-out.
  - **Tooling findings from real-run execution** (surfaced by Scenario 1): polling protocol SSOT consolidation (commit `71eb89fd`) and worktree-guard false-positive fix (commit `a36b13c3`) — both applied in-session per CLAUDE.md zero-deferral, both verified LIVE during Scenario 2's Step 3B nested call.
  - **Scenario 1 + 2 dual-source responses**: saved to `/tmp/ori-tpr-55gWQ2Qz/tp-help-output.md` (Scenario 1) and `/tmp/ori-tpr-HiGdVvg0/tp-help-output.md` (Scenario 2 Step 3B) — tokenized with per-invocation sentinel tokens `4ef518585ae7` and `fb1c71f1b848` respectively, confirming token uniqueness across runs.

- [ ] **Subsection close-out (07.3)** — MANDATORY before section completion:
  - [x] `validate-tp-help-consumers.sh` stub harness exists, runs all 4 stub scenarios in under 60s, exits 0 (committed in `9e8d2080`)
  - [x] impl-hygiene-review REAL integration test passes (Scenario 1 with `ORI_TPR_REVIEWERS=both`) — 2026-04-08, see working notes entry above
  - [x] `.claude/commands/review-plan.md` REAL command-file integration test passes (Scenario 2 with `ORI_TPR_REVIEWERS=both`) — 2026-04-08, PASS-with-scope-note: Step 3B dual-source /tp-help executed end-to-end inside `/review-plan`'s nested skill context, all 7 negative pins passed, 4-agent subagent execution deferred per user authorization (see working notes for the scope decision and its justification). Both pipeline fixes (polling protocol SSOT + worktree-guard false-positive) verified LIVE during this scenario.
  - [x] `.claude/commands/review-plan.md` byte-identity check passes (Scenario 3) — both `git diff --exit-code` AND the frozen §07.1 baseline-hash compare — 2026-04-08, verified during Scenario 1 and re-verified during Scenario 2 close-out
  - [ ] `.claude/skills/create-plan/SKILL.md` REAL integration test passes (Scenario 4 with `ORI_TPR_REVIEWERS=both`, full-run mandatory) across all four internal `/tp-help` call sites  <!-- deferred: user authorized 2026-04-08 to a dedicated session due to ~80-120 min wall-time budget; primary remaining blocker for §07.3 close-out -->
  - [ ] No HTML-comment attribution sentinel leakage into create-plan's final plan overview (grep for all four literal sentinel strings in the written plan files)  <!-- blocked-by-scenario: Scenario 4 (only verifiable by running /create-plan end-to-end) -->
  - [x] No modifications to `impl-hygiene-review`, `.claude/commands/review-plan.md`, or `.claude/skills/create-plan/SKILL.md` required — 2026-04-08, verified via `git diff --exit-code` during both Scenario 1 and Scenario 2 close-outs
  - [ ] Update this subsection's `status` to `complete`  <!-- blocked-by-scenario: Scenario 4 -->
  - [x] **Scenario 1 compiler findings filed as bugs** (not in original plan text but required per CLAUDE.md "file bugs proactively"): BUG-07-006 (LEAK:swallowed-error on ORI_LOG parse fallback), BUG-07-007 (LEAK:shadow-home for ori_llvm parallel tracing init), BUG-07-008 (EXPOSURE for .init() panic), BUG-07-009 (WASTE/BLOAT for tracing-tree dependency) — committed in `b4d2905f`. Both high-confidence findings (BUG-07-006 and BUG-07-007) were caught independently by BOTH codex and gemini via dual-source convergence.
  - [ ] Run `/improve-tooling` retrospectively — should `validate-tp-help-consumers.sh` be folded into `transport-tests.sh` as a new category, or kept as a separate harness? Should the per-invocation random suffix for sentinels be added preemptively, or only after a real collision is observed? If the HTML-comment sentinel format proved too intrusive despite being invisible to renderers (e.g., some downstream consumer's markdown-to-html pipeline strips the comments on the way in and loses attribution), document the failure and file a follow-up bug via `/add-bug` for a more robust attribution scheme.
    - [x] **Polling protocol SSOT consolidation (applied in-session 2026-04-08)**: Scenario 1 execution surfaced that `/tp-help`, `/tpr-review`, and `/review-work` SKILL.md files each inlined their own drifted copy of the polling protocol — tpr-review and review-work were byte-for-byte identical, tp-help had slight wording drift. The inlined protocol used `sleep 300 && status-check.sh` which either auto-backgrounds (silent 5-min periods with no visible output) or produces only relative "T+N min" timestamps with no absolute wall-clock anchor. Operator reported zero real-time visibility during the Scenario 1 dual-invoke wait. Fix applied in-session per `CLAUDE.md` §"ALWAYS improve tooling, NEVER work around it" + zero-deferral: (1) created canonical `.claude/skills/dual-tpr/polling-protocol.md` as SSOT per `impl-hygiene.md` §SSOT / §Algorithmic DRY; (2) replaced the three inline sections in `tp-help/SKILL.md` Step 4.5, `tpr-review/SKILL.md` Polling Protocol, and `review-work/SKILL.md` Polling Protocol with `@.claude/skills/dual-tpr/polling-protocol.md` `@`-include references; (3) rewrote the canonical protocol with mandatory `$RUN/launch.time` artifact (wall-clock anchor via `date | tee`), foreground 75s-sleep poll loop (fits in default 120s Bash foreground timeout, stdout lands immediately), banned-pattern list (no `run_in_background` on polls, no `sleep ≥ 120s`, no relative "T+N" without anchor, no silent waiting between polls); (4) updated `feedback_dual_tpr_polling.md` memory with the new rule and reference to the canonical file. Verification: re-read of the three SKILL.md files confirms `@`-include lines are present; live verification happens at the next dual-source skill expansion (Scenario 2 and beyond). Commit: `71eb89fd`.
    - [x] **Worktree-guard false-positive fix (applied in-session 2026-04-08)**: Scenario 1 execution surfaced that `worktree-guard.sh compare` and `/tp-help` Step 6's inline `diff -u worktree.before worktree.after` BOTH used `diff -q` / `diff -u` semantics that flagged ANY difference between BEFORE and AFTER as "dirty worktree" — including legitimate drift CLEAN-UPS (e.g., Claude committing pre-existing uncommitted edits mid-run). Real Scenario 1 false positive: my `worktree.before` had `M plans/dual-tpr-gemini/section-07-tp-help.md` (the §07.3 read-task findings edit), `worktree.after` became empty when I committed that edit in the polling protocol fix, and the inline check flagged the resulting diff as reviewer-caused drift — which it was NOT. Fix applied in-session per `CLAUDE.md` §"ALWAYS improve tooling, NEVER work around it": (1) updated `worktree-guard.sh compare` mode to use `comm -13 <(sort -u BEFORE) <(sort -u AFTER)` — lines unique to AFTER = new drift, lines removed from BEFORE = legitimate cleanup and NOT a violation; (2) added optional `SAVE_AFTER_FILE` second argument so callers can persist the snapshot as a run artifact; (3) added missing-argument guards and `BEFORE_FILE does not exist` check; (4) updated `/tp-help` Step 6 to delegate to `.claude/skills/dual-tpr/scripts/worktree-guard.sh compare "$RUN/worktree.before" "$RUN/worktree.after"` (SSOT) instead of inlining its own `diff -u` check; (5) verified via 5 unit tests (empty↔empty, before-dirty→after-clean, same↔same, added-file, swapped-file) plus end-to-end test against the actual Scenario 1 `worktree.before`/`worktree.after` files — the corrected check correctly PASSES the Scenario 1 case (drift cleanup is not a violation) while correctly FLAGGING the synthetic "new file modified" case. The fix fulfills the §07.2 retrospective deferral conditional ("if §07.3's real integration tests reveal any need for worktree-guard-inline extraction") — the real integration tests revealed a DIFFERENT need (correct semantics) and the fix is the same architectural move (SSOT the guard into one location). Affects ALL three dual-source consumers: `/tp-help` (inline Step 6 now delegates), `/tpr-review` (via retry wrapper → `worktree-guard.sh compare`), `/review-work` (same).
