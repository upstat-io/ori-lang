...
**Status:** In Progress (reverted — TPR findings unresolved from autopilot session)
...
- [x] Oracle re-derives `ParamContract` per parameter from post-pipeline IR — satisfies mission criterion: "Contract coherence oracle"
...
- [x] Create `compiler/ori_arc/src/aims/verify/oracle.rs`. Add `pub mod oracle;` to `compiler/ori_arc/src/aims/verify/mod.rs`.

- [x] Define the oracle's re-derived contract type:
  ```rust
...
- [x] Implement `derive_param_contracts(func: &ArcFunction) -> Vec<RealizedParamContract>`:
  ```rust
...
- [x] Implement `param_index()` helper that maps an `ArcVarId` to a parameter index (if the var is a parameter). Parameters are the first N variables in the function — check `func.params` to find the mapping.

- [x] Implement `derive_single_param()` that infers `RealizedParamContract` from RC operation counts and use counts.

- [x] Add tests in `compiler/ori_arc/src/aims/verify/oracle/tests.rs`:
...
- [x] Add `Reuse` instruction counting to the oracle walk. A `Reuse` instruction that fails at runtime (slow path: Dec + fresh alloc) is a "missed reuse" — but at IR level, the presence of `Reuse` instructions means the pipeline attempted reuse. The oracle should count:
...
- [x] Derive the oracle's `EffectSummary` from the realized IR:
  ```rust
...
- [x] The oracle comparison for `may_deallocate` must use the POST-second-pass contract. Ensure the oracle runs at the right pipeline point — after `run_second_pass()` in `batch.rs`, not before.
...
- [x] **Subsection close-out (05.2)** — MANDATORY before starting 05.3:
  - [x] All tasks above are `[x]` and the subsection's behavior is verified
  - [x] Update this subsection's `status` in section frontmatter to `complete`
  - [x] **Run `/improve-tooling` retrospectively on THIS subsection** — same protocol as 05.1's close-out, scoped to 05.2's debugging journey.
...
- [x] Implement `verify_coherence(func: &ArcFunction, inferred: &MemoryContract, missed_reuses: u32) -> Vec<CoherenceMismatch>`:
  ```rust
...
- [x] Define compatibility predicates. The oracle comparison must tolerate conservative inference — if the analysis inferred `Unrestricted` (most conservative) but the realized IR only needed `Linear`, that's not a mismatch (the inference was safe). The mismatch direction matters:
...
- [x] Add oracle invocation in `run_aims_pipeline_all()` (in `batch.rs`), after `run_second_pass()`:
  ```rust
...
- [x] Add `ContractCoherenceViolation` variant to `ArcProblem` (or the appropriate error type in `compiler/ori_arc/src/lower/mod.rs`).

- [x] Verify that the oracle passes for all existing tests. Run `ORI_VERIFY_ARC=1 timeout 150 ./test-all.sh` and confirm zero oracle-triggered failures. Any failures are pre-existing contract coherence bugs — file each via `/add-bug` and fix before proceeding.

- [x] Add a regression test that deliberately introduces a contract mismatch (e.g., by hardcoding a param contract field to the wrong value) and verifies the oracle catches it:
  ```rust
  #[test]
...
- [x] **Bug-tracker routing:** Contract coherence violations discovered during implementation should be filed under `plans/bug-tracker/section-04-*.md` (ARC/LLVM subsystem).
...
- [x] `oracle.rs` exists in `compiler/ori_arc/src/aims/verify/` with oracle implementation
- [x] `derive_param_contracts()` correctly walks post-pipeline IR to derive per-param RC behavior
- [x] `may_deallocate` second-pass correction accounted for (oracle runs after `run_second_pass()`)
- [x] Compatibility predicates distinguish unsafe (bug) from conservative (OK) mismatches
- [x] `verify_coherence()` produces clear, actionable diagnostic messages
- [x] Oracle wired into `batch.rs` after second pass, gated by `verify_arc`
- [x] Unsafe mismatches are blocking errors under `ORI_VERIFY_ARC=1`
- [x] `ArcProblem::ContractCoherenceViolation` variant added
- [x] All existing test programs pass oracle: `ORI_VERIFY_ARC=1 timeout 150 ./test-all.sh` green
- [x] Deliberately introduced mismatch caught by regression test
- [x] No regressions: `timeout 150 ./test-all.sh` green
- [x] `timeout 150 ./clippy-all.sh` green
- [x] Plan annotation cleanup: `bash .claude/skills/impl-hygiene-review/plan-annotations.sh --plan 05` returns 0 annotations
