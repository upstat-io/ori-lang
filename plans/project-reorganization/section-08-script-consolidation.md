...
**File(s):** `scripts/dev/` (new), `scripts/diagnostics/` (new)
...
- [ ] **Pattern A: `./clippy-all.sh` → `./scripts/dev/clippy-all.sh`** (~175 matches)
  ...
  ```bash
  cd /home/eric/projects/ori_lang
  rg -l '\./clippy-all\.sh' > /tmp/sweep-pattern-a-files.txt
  wc -l /tmp/sweep-pattern-a-files.txt
  # Expected: ~108 files
  ```
  ...
  grep -E '^(target|\.git|scripts/dev/clippy-all\.sh)' /tmp/sweep-pattern-a-files.txt || echo "no unexpected files"
  ```
  ...
  done < /tmp/sweep-pattern-a-files.txt
  ```
...
- [ ] **Pattern B: `./llvm-build.sh` → `./scripts/dev/llvm-build.sh`** (likely 0 matches)
  ...
  ```bash
  rg -l '\./llvm-build\.sh' > /tmp/sweep-pattern-b-files.txt
  wc -l /tmp/sweep-pattern-b-files.txt
  ```
...
- [ ] **Pattern D: `./llvm-test.sh` → `./scripts/dev/llvm-test.sh`** (~30 matches)
  ...
  ```bash
  rg -l '\./llvm-test\.sh' > /tmp/sweep-pattern-d-files.txt
  wc -l /tmp/sweep-pattern-d-files.txt
  # Expected: ~16 files

  while read -r file; do
    sed -i 's|\./llvm-test\.sh|\./scripts/dev/llvm-test\.sh|g' "$file"
  done < /tmp/sweep-pattern-d-files.txt
...
- [ ] **Pattern E: `diagnostics/` → `scripts/diagnostics/`** (explicit allow-list scoped; DO NOT use deny-list)
  ...
  **Step E.2 — Build the explicit allow-list of files to sweep:**
  ...
  } > /tmp/sweep-pattern-e-allowlist.txt
  wc -l /tmp/sweep-pattern-e-allowlist.txt
  ```
  - [ ] Allow-list file count captured.
  - [ ] Manually spot-check the allow-list — `head -50 /tmp/sweep-pattern-e-allowlist.txt` — to verify no compiler/library/tests files leaked in AND no `scripts/dev/` or `scripts/diagnostics/` files leaked in.
  - [ ] **Defensive check**: `grep -E '^(compiler|library|tests|scripts/dev|scripts/diagnostics)/' /tmp/sweep-pattern-e-allowlist.txt` — must return empty. If any match: STOP, fix the allow-list generation.
  ...
  **Step E.3 — Narrow the allow-list to files that actually contain `diagnostics/` (avoid touching files that don't need changes):**
  ...
  xargs -a /tmp/sweep-pattern-e-allowlist.txt rg -l 'diagnostics/' 2>/dev/null \
    | sort -u > /tmp/sweep-pattern-e-files.txt
  wc -l /tmp/sweep-pattern-e-files.txt
  ```
  - [ ] Review the final list manually:
    ```bash
    cat /tmp/sweep-pattern-e-files.txt
    ```
  ...
  - [ ] **If ANY file in `/tmp/sweep-pattern-e-files.txt` is under `compiler/`, `library/`, or `tests/`: STOP. The allow-list is leaking. Investigate before proceeding.**
  ...
  **Step E.5 — Apply the sed substitution to the allow-listed files ONLY.** Use multiple scoped patterns rather than a single catch-all to minimize false-positive risk:
  ...
  while read -r file; do
    ...
    sed -i 's|^diagnostics/|scripts/diagnostics/|g' "$file"
  done < /tmp/sweep-pattern-e-files.txt
  ```
  ...
  **Step E.9 — Post-move sweep of `scripts/diagnostics/` self-references.** (TPR-XX-004-codex fix — 2026-04-11.) The moved diagnostic scripts and `README.md` contain internal self-references that say `diagnostics/ir-dump.sh ...` etc. Pattern E's allow-list deliberately EXCLUDES `scripts/diagnostics/` (to avoid scope creep), so those self-references survive unchanged after §08.4's move. This creates a DRIFT finding: the canonical home is `scripts/diagnostics/` but the scripts' own help text still instructs users to run `diagnostics/...`. Fix with a scoped post-move sweep that ONLY touches `scripts/diagnostics/**`.
  ...
  rg -l 'diagnostics/' scripts/diagnostics/ > /tmp/sweep-pattern-e9-files.txt
  wc -l /tmp/sweep-pattern-e9-files.txt
  cat /tmp/sweep-pattern-e9-files.txt
  ```
  ...
  ```bash
  while read -r file; do
    # Match diagnostics/ in path-like contexts: after space, backtick, or #! / start-of-line
    # Use the same scoping as Pattern E to avoid rewriting prose
    sed -i 's|`diagnostics/|`scripts/diagnostics/|g; s|\./diagnostics/|./scripts/diagnostics/|g; s| diagnostics/| scripts/diagnostics/|g; s|^diagnostics/|scripts/diagnostics/|g' "$file"
  done < /tmp/sweep-pattern-e9-files.txt
  ```
...
| C7 | test-code reference (`compiler/ori_llvm/tests/aot/util/aot.rs:168`) | In-repo Rust test code invoking scripts |
| C8 | negative-pin (deliberate test failure — wrapper must propagate non-zero exit) | Wrapper exit-code transparency |

**Cells that apply per script type:**
- **Wrapper scripts (test-all, full-check, fmt-all, build-all)**: C1, C2, C3, C4, C5, C8. C6 for full-check only (it's the only one that chains internally). C7 only if the test-code reference hits this specific script.
- **Cleanly-moved scripts (clippy-all, llvm-*)**: C2, C3, C8. C1 MUST NOT work (no root copy), C4/C5 don't apply (not in lefthook), C6 only for clippy-all (called by full-check via `$SCRIPT_DIR`), C7 for `llvm-test.sh`.
...
- [ ] **C3 — cross-directory invocation (PWD assumption test, 4 cells — wrappers only)**
  ...
  Wrappers assume cwd = repo root (test-all.sh writes `target/test-all-results.json` as a relative path, etc.). Test invocation from a subdirectory:
  ```bash
  cd /home/eric/projects/ori_lang/compiler
  timeout 150 ../test-all.sh 2>&1 | tail -10; echo "exit: $?"
  timeout 30 ../fmt-all.sh 2>&1 | tail -10; echo "exit: $?"
  cd /home/eric/projects/ori_lang
  ```
...
- [ ] **C7 — test-code reference (1 cell — compiler/ori_llvm/tests/aot/util/aot.rs:168)**

  Verify the Rust test code that invokes a shell script from within a test still resolves correctly after the reference sweep:
  ```bash
  grep -n '\./test-all\.sh\|\./llvm-test\.sh\|scripts/dev/' compiler/ori_llvm/tests/aot/util/aot.rs
  # Expected: shows either the original ./test-all.sh reference (if wrapper preserves it)
  # or the updated ./scripts/dev/llvm-test.sh reference (if Pattern D caught it)
  ```
  - [ ] The test code reference is correctly resolved
  - [ ] Run the specific AOT test that exercises this path:
    ```bash
    timeout 150 cargo test -p ori_llvm --test aot_smoke 2>&1 | tail -20
    ```
  - [ ] Exit 0
...
- [ ] **Matrix completeness check:** total cells exercised = 4 (C1) + 4 (C1-negative) + 8 (C2) + 4 (C3, with documented limitation) + 4 (C4) + 2 (C5) + 1 (C6) + 1 (C7) + 1 (C8) + 4 (C9, SKIP unsupported) = **33 cells**.
  ```bash
  ...
  ```
  - [ ] Document every SKIPPED cell. Acceptable skips: C5 when lefthook not locally installed (will run at commit time), C7 when the aot.rs line doesn't invoke a §08-affected script (in which case this cell is N/A), C9 PATH invocation (all 4 cells — unsupported per TPR-XX-002-codex fix).
  - [ ] Any FAILED cell that is NOT a documented limitation (C3 per Finding 8, C9 per TPR-XX-002-codex) blocks §08.8 commit.
...
- [ ] **Commit everything in §08 atomically with EXPLICIT per-file path staging (no `git add -A`, no `git add <directory>/`).**
  ...
  **Why explicit per-file staging (TPR-XX-001-codex + TPR-XX-002-gemini fix, 2026-04-11):** Both directory-wide adds (`git add .claude/`) and awk-based glob adds absorb unrelated concurrent work. The correct allow-list is the set of files §08.5's sweep patterns ACTUALLY touched — which we already captured in `/tmp/sweep-pattern-a-files.txt`, `/tmp/sweep-pattern-d-files.txt`, and `/tmp/sweep-pattern-e-files.txt` during Step E.2/E.3. Reuse those files as the staging source of truth. Any file NOT in those lists is either (a) unchanged, or (b) concurrent dirt that must NOT be absorbed into the §08 commit.
  ...
  # 4. Diagnostic moves (from 08.4 — explicit per-file, no glob)
  ...
  # fixtures subdir
  git add -u diagnostics/fixtures/ 2>/dev/null || true
  # Staged new locations (post-move)
  git add scripts/diagnostics/_common.sh scripts/diagnostics/arc-dump.sh \
          ...
          scripts/diagnostics/fixtures/ scripts/diagnostics/_repo-root.sh
  ...
  if [ -s /tmp/sweep-pattern-a-files.txt ]; then
    xargs -a /tmp/sweep-pattern-a-files.txt git add --
  fi
  if [ -s /tmp/sweep-pattern-d-files.txt ]; then
    xargs -a /tmp/sweep-pattern-d-files.txt git add --
  fi
  if [ -s /tmp/sweep-pattern-e-files.txt ]; then
    xargs -a /tmp/sweep-pattern-e-files.txt git add --
  fi
  if [ -s /tmp/sweep-pattern-e9-files.txt ]; then
    xargs -a /tmp/sweep-pattern-e9-files.txt git add --
  fi
...
- [ ] **Post-stage catch-all gate (TPR-XX-002-gemini fix, 2026-04-11).** Verify NO unstaged changes remain — if any appear, they're either a missed sweep target (must be added to staging) or concurrent dirt (must be shelved before commit). This guards the gap where Pattern A/D could touch a file in `.github/`, `docker/`, `editors/`, or other directory NOT anticipated by explicit staging:
  ...
  if [ -n "$unstaged" ]; then
    ...
    echo ""
    echo "Classify each:"
    echo "  - If it's a missed sweep target (rg pattern hit but not in /tmp/sweep-pattern-*.txt):"
...
- [x] `[TPR-XX-001-codex][high]` `plans/project-reorganization/section-08-script-consolidation.md:1341` — Remove the LEAK in atomic staging scope.
  ...
  Required plan update: Narrow staging to an explicit per-file allow-list derived from the paths Pattern A/D/E actually touched. Use `/tmp/sweep-pattern-{a,d,e,e9}-files.txt` as the staging source of truth.
  Basis: fresh_verification. Confidence: high.
  Resolved: Fixed on 2026-04-11. Rewrote §08.8 staging to use `xargs -a /tmp/sweep-pattern-a-files.txt git add --` (and similarly for Pattern D/E/E9). Removed all `git add .claude/` / `git add .codex/` / `git add .gemini/` directory adds. Removed the awk-based globs for plans/ and docs/. Every staged path is now an explicit filename derived from §08.5's own sweep file lists.
...
- [x] `[TPR-XX-004-codex][medium]` `plans/project-reorganization/section-08-script-consolidation.md:800` — Sweep the remaining DRIFT in diagnostics self-references.
  ...
  Basis: fresh_verification. Confidence: high.
  Resolved: Fixed on 2026-04-11. Added Step E.9 (`Post-move sweep of scripts/diagnostics/ self-references`) to §08.5 with explicit scoping to `scripts/diagnostics/**`, the same regex patterns as Pattern E, a negative-pin (Step E.10) that verifies the E.9 sweep did NOT escape `scripts/diagnostics/`. The `/tmp/sweep-pattern-e9-files.txt` output is used as an explicit staging source in §08.8's per-file add loop.
