...
- [ ] **Gitignore drift bug fixed** — `git check-ignore -v build/debug/simplest_crash.ll` returns a matched rule, `git ls-files build/` returns empty, and the `.gitignore` has a root-anchored `/build/` rule coexisting with `**/build/` without breaking the existing negative rules for `compiler/oric/src/commands/build/`. *(Delivered by: §03.)*
...
│   build/debug/simplest_crash.ll             ← leaked past **/build/ (root /build/ not anchored)
...
|-----|-----------|-------------|--------|
| `build/debug/simplest_crash.ll` + `build/debug/test_simple` tracked despite `**/build/` gitignore | `**/build/` does not match root-level `/build/` in gitignore pattern resolution; these files were committed before the nested-only rule was the only protection | §03 | Not Started |
