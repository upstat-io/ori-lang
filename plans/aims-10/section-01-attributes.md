...
- [ ] In `generate_main_wrapper()`, after declaring the function with `declare_function("main", ...)`, add `noundef` to the return value attribute
...
- [ ] Test: Write a Rust unit test `main_wrapper_has_noundef_return` in `function_compiler/tests.rs` that compiles a trivial `@main () -> void` and checks the main wrapper's return attribute
- [ ] Test: `ORI_DUMP_AFTER_LLVM=1 ./target/debug/ori run --compile plans/code-journeys/01-arithmetic.ori 2>&1 | grep 'define.*@main'` shows `noundef` on return
...
- [ ] Audit: For each non-nounwind function in J5, J9, J10, J13, determine WHY it's not nounwind
  - Run `ORI_DUMP_AFTER_LLVM=1` and check for `invoke` instructions
  - If ALL `invoke` targets are `nounwind` callees, the function should be `nounwind`
  - If the function contains `invoke` to a genuinely unwinding callee, it correctly lacks `nounwind`
- [ ] If the posthoc pass misses functions: fix `apply_posthoc_nounwind()` in `nounwind.rs` to also check lambda functions and trampoline functions
  - Note: lambdas ARE in `codegen_ctx.functions` (registered by `declare_and_process_lambda`). The gap is closure wrappers `_ori_partial_N` — see 01.6 for the fix.
- [ ] If functions are correctly non-nounwind (they genuinely can unwind): the attribute gap is unfixable and the scoring tool (`.claude/skills/code-journey/extract-metrics.py`) should not count `nounwind` as applicable for functions that contain `invoke` to unwinding callees. Update `.claude/skills/code-journey/attribute_metrics.py` to skip `nounwind` check for functions with `invoke` to non-nounwind callees.
  - `.claude/skills/code-journey/attribute_metrics.py` already has a nounwind applicability rule at line ~133: it only checks true leaf functions (no user-function calls, no closures). Functions with `invoke` are NOT checked for nounwind. The issue is likely that non-leaf nounwind functions (proven by the compiler's fixed-point analysis) are not given credit. The fix may need a second rule: "compiler-proven nounwind on non-leaf functions is correct" (check that all invoke targets in the IR are `nounwind`-attributed).
- [ ] Verify: Attribute compliance improves for J5, J9, J10, J13
...
- [ ] In `declare_function_llvm_with_extra_params()`, apply `noundef` to both `ParamPassing::Indirect` and `ParamPassing::Reference` pointer params
  - The pointer is always a valid, defined address — not poison or undef
  - `Reference` is produced when borrow inference determines `Ownership::Borrowed` for non-Scalar types
  - This is independent of `nonnull`/`dereferenceable`/`readonly` which are also applicable
  - The change goes in the `else if !matches!(param.passing, ParamPassing::Void)` branch (line ~240-246 of `function_compiler/mod.rs`). Currently that branch only adds `readonly` for borrowed params. Add `self.builder.add_noundef_param_attribute(func_id, nidx);` before the readonly check, so ALL pointer params (Indirect + Reference) get `noundef`.
- [ ] Affected functions: `@_ori_area` (J4), `Shape$eq` (J11), `@_ori_count_items` (J10)
- [ ] Test: Update `indirect_params_no_noundef` test in `function_compiler/tests.rs` → rename to `indirect_params_have_noundef`
  - If no such test exists, write a new `indirect_params_have_noundef` test that declares a function with an Indirect param and verifies `noundef` is present on the pointer param
- [ ] Verify: `timeout 150 ./test-all.sh` green
