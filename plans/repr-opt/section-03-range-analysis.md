...
third_party_review:
  status: findings
  updated: 2026-03-26
  triage_note: "TPR-03-034/035 accepted on 2026-03-26 — implementation tasks added to §03.5. TPR-03-034: thread call_result_narrowings through process_terminator() for Invoke. TPR-03-035: pass remaining budget into process_recursive_scc(). Prior TPR-03-028..033 resolved."
...
- [ ] **[TPR-03-034] Thread `call_result_narrowings` through `process_terminator()` for Invoke dst vars**: Currently `process_terminator()` in `fixpoint/terminator.rs` handles `Invoke` by calling `transfer_known_call()` only — it never applies the callee return-range narrowing from `call_result_narrowings`. Fix: add `call_result_narrowings: &FxHashMap<ArcVarId, ValueRange>` parameter to `process_terminator()`, and after computing the Invoke dst range via `transfer_known_call`, apply `meet` with any entry in the narrowings map. Update `run_forward_iteration()` and `run_narrowing_pass()` callers. Required tests:
  - [ ] Semantic pin: Invoke dst → derived local propagation (mirrors `callee_return_derived_local_propagates` but using Invoke terminator)
  - [ ] Semantic pin: Invoke dst → downstream callee parameter propagation (mirrors `callee_return_derived_local_forwards_to_callee_param` but using Invoke)
- [ ] **[TPR-03-035] Pass remaining total budget into `process_recursive_scc()`**: Currently `process_recursive_scc()` only checks `config.max_scc_iterations` (per-SCC limit) but ignores the remaining cross-SCC budget. Fix: add `remaining_budget: usize` parameter, use `min(config.max_scc_iterations, remaining_budget)` as the effective cap. When budget is exhausted mid-SCC, widen all members to Top (same as current budget-exceeded path). Required tests:
  - [ ] Semantic pin: `max_total_scc_iterations` < `max_scc_iterations` → SCC stops at total budget, not per-SCC cap
...
- [ ] `[TPR-03-034][high]` `compiler/ori_repr/src/range/fixpoint/terminator.rs:32` — Return-range feedback reruns still do not narrow `Invoke` destinations, despite the implementation and plan both claiming `Apply` and `Invoke` coverage.
  ...
  Accepted: Validated on 2026-03-26. Implementation task added to §03.5 (thread `call_result_narrowings` through `process_terminator()` + 2 Invoke semantic-pin tests).

- [ ] `[TPR-03-035][medium]` `compiler/ori_repr/src/range/signatures/mod.rs:126` — `max_total_scc_iterations` is only enforced between SCCs, so one recursive SCC can exceed the configured total-work cap.
  ...
  Accepted: Validated on 2026-03-26. Implementation task added to §03.5 (pass remaining budget into `process_recursive_scc()` + semantic-pin test).
