...
- [ ] **Loop variable narrowing convergence**: The narrowing pass cannot recover bounded ranges for loop counter phis. Root cause: widening produces `[0, MAX]`, the false-branch refinement is `[-MIN, 9]` (unconstrained lower bound), and `join([0, 0], [-MIN+1, 10]) = [-MIN+1, 10]` — the entry's tight lower bound (0) is swamped by the body's wide range. Threshold-guided widening helps the upper bound (10 instead of MAX) but doesn't fix the lower bound. **Needs**: loop-aware narrowing that preserves the entry predecessor's lower bound, or demand-driven analysis (like LLVM's LazyValueInfo). Improvements already landed: (1) inline body instruction updates in narrowing pass, (2) `refine_from_branch` copy-chain tracing (resolves %6→%4 through Var copies), (3) refinement propagation through single-predecessor jump chains, (4) threshold-guided widening (using comparison constants as landmarks). These are correct but insufficient for the lower-bound issue. **Blocking**: §04.4 Phase B IR pin tests `test_phase_b_ir_pin_loop_counter_phi` and `test_phase_b_ir_pin_loop_sext` (marked `#[ignore]`).
