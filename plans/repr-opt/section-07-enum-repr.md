...
- [x] `[TPR-07-006][high]` `compiler/ori_llvm/src/codegen/derive_codegen/enum_bodies/enum_eq.rs:124` — **Derived enum `Eq` still treats zero-sized payload fields as occupied i64 slots, so `#derive(Eq)` panics on enums like `A(u: void, x: int) | B`.**
  Resolved: Fixed on 2026-03-30 (commit e0d360ce). Added `variant_non_void_field_types()` helper that filters void/Never fields before payload traversal. Applied to all 3 ForEachField-strategy derives (Eq, Comparable, Hashable). Verified: `#derive(Eq) type E = A(u: void, x: int) | B` compiles and runs correctly in both interpreter and AOT. Existing AOT tests in `enum_zst.rs` cover the fix. All tests pass.
