...
- [ ] `[TPR-07-011][high]` `compiler/ori_llvm/src/codegen/arc_emitter/instr_dispatch.rs:238` / `compiler/ori_llvm/src/codegen/arc_emitter/rc_helpers.rs:506` — **Moving an iterator payload out of a tagged-pointer enum and then consuming it double-frees the iterator.**
  Reproduced on 2026-04-06 with:
  ```ori
  ...
  ```
  `timeout 150 ./target/debug/ori build /tmp/review_tagged_iter_match.ori -o /tmp/review_tagged_iter_match_bin && ORI_CHECK_LEAKS=1 /tmp/review_tagged_iter_match_bin` aborts with `free(): double free detected in tcache 2` (exit 134). `ORI_DUMP_AFTER_ARC=1` shows the ownership shape: `%11 = Project %7.1`, `Invoke @count(%12 [own])`, then `RcDec %7 [InlineEnum]` on the original enum after the arm. The tagged-pointer `Project` path simply decodes the payload pointer and hands it to the destination variable, while the tagged-pointer RC/drop path still decodes the same pointer from the original enum and drops it at scope exit. For RC-managed pointer payloads an extra retain can mask this; for iterator payloads (`RcStrategy::Iterator` / `ori_iter_drop`) it becomes an immediate double-free. The current AOT matrix only pins the "goes out of scope unused" case in `compiler/ori_llvm/tests/aot/iterator_drop.rs`; it does not cover `match`/`Project` + consume.

- [ ] `[TPR-07-012][low]` `compiler/ori_arc/src/aims/emit_rc/unwind_cleanup/mod.rs:111` / `compiler/ori_arc/src/aims/emit_rc/unwind_cleanup/tests.rs:92` — **The synthetic unwind-path `ori_iter_drop` instructions still hardcode `ArgOwnership::Borrowed`, contradicting the new SSOT that `ProtocolBuiltin::IterDrop` is `Owned`.**
  This review validated that the current tests still expect the stale ownership (`invoke_indirect_resume_inserts_iter_drop` asserts `Borrowed`). It is not causing the crash above because `add_invoke_unwind_cleanup()` runs after RC insertion, so the stale metadata is currently inert. But it is now a shadow contract source inside the ARC pipeline: any future pass reusing these synthetic `Apply` instructions as ownership evidence would reason from the pre-TPR-07-008 semantics. The fix should make the injected unwind cleanup agree with `ProtocolBuiltin::IterDrop.arg_ownership()` and update the unit test accordingly.
