...
- [ ] `[TPR-06-005][high]` `compiler/ori_llvm/src/codegen/type_info/type_size.rs`, `compiler/ori_arc/src/lower/control_flow/type_layout.rs`, `compiler/ori_arc/src/lower/control_flow/for_yield.rs` — LLVM-side element sizing now includes struct padding, but the for-yield lowerer still computes list element sizes as a raw sum of field sizes. Yielding reordered mixed-field structs into a list will allocate/copy too few bytes via `ori_list_new`/`ori_list_push`, truncating the stored element and risking memory corruption.
  Still reproducible on 2026-03-30. `lower_for_yield_iterator()` bakes `elem_size` from `compute_elem_size()` / `pool_type_store_size()` (`compiler/ori_arc/src/lower/control_flow/for_yield.rs:164-178,335-345`), while LLVM only rewrites the baked constant for narrowed `int` accumulators (`compiler/ori_llvm/src/codegen/arc_emitter/apply.rs:236-255`). A concrete reproducer collecting `Record { flag: bool, name: str, count: int }` through `for x in xs yield ...` returns `5` in the interpreter but crashes AOT with SIGSEGV (`exit=139`) under `timeout 150 diagnostics/dual-exec-debug.sh --no-color /tmp/tpr_06_005_repro.ori`.
