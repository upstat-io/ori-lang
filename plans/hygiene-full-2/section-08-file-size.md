...
status: not-started
reviewed: false
goal: "Split all 69 production files exceeding the 500-line limit into focused submodules, and decompose 31+ functions exceeding the 100-line limit" <!-- reviewed: cohesion fix — 69 files not 58; added function-level violations -->
depends_on: ["01", "02", "03", "04", "05", "06", "07"] <!-- reviewed: cohesion fix — was missing 03 and 07; all DRY+annotation sections should complete before file splitting -->
third_party_review:
  status: none
  updated: null
sections:
...
**Status:** Not Started
**Goal:** Split all 69 production source files exceeding the 500-line limit into focused submodules, and decompose 31+ functions exceeding the 100-line limit into helper functions. Each resulting file should be under 500 lines with a single clear responsibility; each function under 100 lines (target <50). <!-- reviewed: cohesion fix — 69 files not 58; added function-level violations -->
...
- [ ] For each file still >500 lines: identify logical split points — look for section comments, distinct impl blocks, or groups of related functions <!-- reviewed: executability fix — clarified how to find split points -->
- [ ] Split files, update `mod.rs` declarations, verify imports
- [ ] Use `#[cfg(test)] mod tests;` pattern for test files
- [ ] `timeout 150 cargo test -p ori_types` after each split to catch broken imports immediately <!-- reviewed: executability fix — intermediate test gate -->
- [ ] **Priority order:** Start with the worst offenders (operators.rs 799, registry/traits/mod.rs 765, unify/mod.rs 761) — these have the clearest logical split points <!-- reviewed: executability fix -->
...
- `arc_emitter/terminators.rs` — 745 lines <!-- reviewed: cohesion fix — was listed as 518, actual is 745 -->
- `function_compiler/define_phase.rs` — 675 lines <!-- reviewed: cohesion fix — missing from original list -->
...
- `ir_builder/checked_ops.rs` — 543 lines <!-- reviewed: cohesion fix — missing from original list -->
- `arc_emitter/builtins/debug_helpers.rs` — 534 lines
- ori_arc: 8 files (state_map 646, aims_pipeline 590, rewrite 573, verify 559, lattice 552, interprocedural 536, transfer 524, extract 517) <!-- reviewed: accuracy fix -->

- [ ] Re-measure after Section 05 DRY work
- [ ] For `runtime_functions.rs` (1606 lines): validate data table exemption. Exemption criteria: file contains ONLY `const`/`static` declarations and their type definitions, with no function bodies containing logic (trivial constructors OK). If it contains runtime logic, dispatch tables, or non-trivial functions, split. Add explicit `// FILE SIZE EXEMPTION: pure static data table — N entries` comment at file top. <!-- reviewed: cohesion fix — made exemption criteria concrete -->
...
- [ ] `ori_patterns/src/errors/mod.rs` (1018 lines) — worst offender. Contains: type definitions (`ControlAction`, `EvalError`, `EvalErrorKind` enum with ~30 variants), factory functions (~40 `pub fn`), and `Display`/`From` impls. Split into:
  ...
  <!-- reviewed: executability fix — specified concrete split structure -->
...
- `ori_ir/src/arena/range_builders.rs` — 608 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_parse/src/grammar/expr/patterns/match_patterns.rs` — 595 lines <!-- reviewed: cohesion fix — was vague "Plus 5 more" -->
...
- `oric/src/test/runner/mod.rs` — 572 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_rt/src/rc/mod.rs` — 554 lines <!-- reviewed: cohesion fix — missing from original list -->
- `oric/src/problem/codegen/mod.rs` — 551 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_diagnostic/src/diagnostic/mod.rs` — 547 lines <!-- reviewed: cohesion fix — missing from original list -->
- `oric/src/test/runner/llvm_backend.rs` — 545 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_parse/src/grammar/item/function/mod.rs` — 532 lines <!-- reviewed: cohesion fix — was vague -->
- `ori_ir/src/arena/mod.rs` — 530 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_fmt/src/formatter/inline.rs` — 520 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_rt/src/string/methods/mod.rs` — 517 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_fmt/src/spacing/category.rs` — 516 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_rt/src/lib.rs` — 512 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_fmt/src/formatter/broken.rs` — 511 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_ir/src/ast/expr.rs` — 510 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_ir/src/canon/expr.rs` — 507 lines <!-- reviewed: cohesion fix — missing from original list -->
- `ori_ir/src/canon/arena.rs` — 507 lines <!-- reviewed: cohesion fix — missing from original list -->
...
<!-- reviewed: cohesion fix — this was tracked as a metric in 00-overview.md but had no section addressing it. CLAUDE.md mandates "functions < 100 lines (target < 50)". -->
...
<!-- reviewed: executability fix — the original awk command was unreliable (nested braces break it); replaced with practical approach -->
