...
- [ ] `[TPR-03-005][major]` `compiler/ori_arc/src/lower/control_flow/type_layout.rs:50` — `pool_type_store_size` catch-all `_ => 8` returns wrong size for closures (`Tag::Function` = 16 bytes: `{fn_ptr, env_ptr}`), custom enums (`Tag::Enum` = tag + max variant payload, variable), `Tag::Ordering` (1 byte), and other unhandled tags. This function is called via `compute_elem_size()` in for-yield codegen (`for_yield.rs:380`, `for_yield_option.rs:78`) to set `elem_size` for `ori_list_new` and `ori_list_push`. A `for x in list yield closure_expr` passes `elem_size=8` to `ori_list_push` but the closure is 16 bytes — only 8 bytes are copied, and the second push overwrites the first element's second half. Buffer overrun on subsequent pushes. The existing test (`tests.rs:540-595`) only tests explicitly-handled tags.
  Evidence: `type_layout.rs:50` shows `_ => 8` catch-all. `ori_llvm` `TypeInfo::Function` returns `Some(16)` at `info.rs:213`. No test covers for-yield with closure or custom enum body results.
  Impact: Silent data corruption for `for x in list yield (y -> x + y)` and similar patterns yielding closures or multi-word enums in AOT. Closure env_ptr is truncated, causing use-after-free or garbage data when the result list is consumed.
  Required fix: Add explicit handling for `Tag::Function` (16), `Tag::Enum` (compute from tag + max variant), `Tag::Ordering` (1), `Tag::Iterator`, `Tag::Range`, `Tag::DoubleEndedIterator`. Change catch-all to `unreachable!` or `debug_assert!` with explicit `Tag::Int | Tag::Float | Tag::Duration | Tag::Size => 8`. Add AOT tests for `for x in list yield closure` and `for x in list yield SomeEnum.Variant(x)`.
  Root cause: SYSTEMIC — `pool_type_store_size()` and `TypeLayoutResolver::type_store_size()` compute the same value from different abstractions but diverge. TODO at `type_layout.rs:14` acknowledges this. Extract shared layout logic to `ori_ir`.

- [ ] `[TPR-03-006][minor]` `compiler/ori_arc/src/lower/control_flow/for_yield_option.rs:131-134` — Guard-skip exit args use `filter_map(|&(name, _, _)| self.scope.lookup(name))` which silently drops names that fail scope lookup, producing fewer args than the exit block expects (exit block has one param per `mut_info` entry). Same pattern at lines 171-173 (body exit). The sibling `for_yield.rs:286-287` uses the strictly safer `header_mut_params.iter().map(|&(_, _, param)| param)` which is infallible. Same fragile pattern in `mod.rs:269-272,293-294,337-340,355-356` for break/continue lowering.
  Evidence: `for_yield_option.rs:88-93` creates exit_block with `mut_info.len()` params. `filter_map` at line 131-134 can produce fewer than `mut_info.len()` args if any lookup fails. `for_yield.rs:287` avoids this by using pre-captured block params.
  Impact: Low in practice (scope lookups for pre-captured mutable bindings should always succeed), but a latent invariant violation. A future refactoring changing scope semantics could cause LLVM verification failure from args/params count mismatch.
  Required fix: Use the `header_mut_params` pattern from `for_yield.rs` in all guard-skip and break/continue paths. Extract a shared helper `build_mutable_jump_args()`.
